...
| `./install` | Install **`tddy-daemon`**, **`tddy-coder`**, **`tddy-tools`** as a systemd service: `sudo ./install --systemd` (optional `--build` runs `./release`). Path overrides: `INSTALL_PREFIX`, `INSTALL_BIN_DIR`, `INSTALL_CONFIG_DIR`, `INSTALL_SYSTEMD_DIR`; test harness: `INSTALL_NO_SYSTEMCTL=1` skips root check and `systemctl`. |
