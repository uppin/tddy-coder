...
| Script | Purpose |
|--------|---------|
| `./dev` | Enter nix dev shell with profile (persists across `nix gc`). With args: run command inside shell, e.g. `./dev cargo test` or `./dev echo "Hello"`. Also wires the **build cache** — see `scripts/build-cache-env.sh` below. |
...
| `scripts/build-cache-env.sh` | Resolve this host's **sccache** settings and print them as `export` lines; `./dev` and `.envrc` eval it. The host names a backend in **`~/.tddy/build-cache.yaml`** (`local` — a directory, `redis` — shared, `github-actions`, or `off`); **`TDDY_BUILD_CACHE`** names one directly and skips the file, which is how CI is wired — CI uses **redis** (endpoint in `ci.yml`, password from the `SCCACHE_REDIS_PASSWORD` repo secret), deliberately not the Actions cache, which is at GitHub's 10 GB ceiling. No file means sccache is simply off — there is no implicit local cache. Run it bare to see what a host resolved to; `./dev sccache --show-stats` for the hit rate. See [docs/dev/guides/build-cache.md](docs/dev/guides/build-cache.md). |
