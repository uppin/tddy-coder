...
| `scripts/build-cache-env.sh` | Resolve this host's **sccache** settings and print them as `export` lines; `./dev` and `.envrc` eval it. The host names a backend in **`~/.tddy/build-cache.yaml`** (`local` — a directory, `redis` — shared, `github-actions`, or `off`); **`TDDY_BUILD_CACHE`** names one directly and skips the file, which is how CI is wired. No file means sccache is simply off — there is no implicit local cache. Run it bare to see what a host resolved to; `./dev sccache --show-stats` for the hit rate. See [docs/dev/guides/build-cache.md](docs/dev/guides/build-cache.md). |
