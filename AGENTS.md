...
| `./web-dev` | Start **`tddy-daemon`** (see **`DAEMON_CONFIG`** / **`dev.daemon.yaml`**) and the **`tddy-web`** Vite dev server with **`/rpc`** proxy. **`--headless`** runs the backend alone. **`--build`** builds every binary the running stack can *invoke* — `DEV_RUNTIME_PACKAGES` in **`scripts/dev-runtime-binaries.sh`**, the same set `./install` ships beside the daemon — because each is resolved as a sibling of the daemon binary and then as a bare name on `PATH` with **no existence check**, so a missing one is not a build error but a session that will not start. **`--resolve-private-repo`** installs the JS dependencies through the local private npm registry (`bun run local-registry-install`, `LOCAL_REGISTRY_URL`) instead of the public one. Both flags are consumed here; every other argument passes through to `tddy-daemon`. See [docs/ft/web/local-web-dev.md](docs/ft/web/local-web-dev.md). |
| `./desktop-dev` | Start **Tddy Desktop** in development: `tauri dev` (the app's own process hosts a `tddy-daemon`) plus the **`tddy-web`** Vite dev server, against the checkout's **`dev.desktop.yaml`** and **`tmp/.tddy`** so a dev run never touches an installed app's `~/.tddy`. Takes the same **`--build`** and **`--resolve-private-repo`** as `./web-dev` — `--build` matters more here, since `tauri dev` builds only `tddy-desktop` and never the siblings the hosted daemon invokes. Every other argument passes through to `tauri dev`. |
