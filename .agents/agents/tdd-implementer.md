...
**Mandatory reading before implementing**: `.agents/skills/fluent-tests/references/generic-guidelines.md`
...
`.agents/skills/tdd/SKILL.md` for the TDD cycle, `.cursor/rules/testing-practices.mdc` and
...
### 0. Read the implementation plan, if one was given

`/red` may hand you an implementation plan alongside the failing tests. When it does, read it
first and extract the file structure to create, the type definitions, the core algorithm, the
error-handling strategy, and the architectural decisions already made.

Treat the plan as **guidance, not a contract — the tests are the source of truth.** Where the
tests pin an API the plan didn't anticipate, follow the tests and adapt the plan. Keep the plan's
architectural decisions wherever they don't conflict, and document every deviation and why in
your final report. If no plan was provided, derive the approach from the tests alone.
...
- Whether a plan was provided, whether you followed it, and every adjustment you made to it with
  the reason — or "no plan provided; approach derived from tests."
...
**Handoff**: if everything passes, the `refactor` agent (`.agents/agents/refactor.md`) is the next
step for code-quality improvement. If tests are still failing, the next step is investigating the
root cause — never forcing passage.
