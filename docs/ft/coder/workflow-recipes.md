...
- **Structured submit:** **`WorkflowRecipe::goal_requires_tddy_tools_submit`** defaults to **`true`** for TDD-style goals; **`FreePromptingRecipe`** returns **`false`** for **`prompting`** so a turn can complete from normal agent output without **`tddy-tools submit`** (open-ended chat with backends that do not relay the tool). On this no-submit path, **`BackendInvokeTask`** persists the response into **`context["output"]`** (same as the submit-success path) so downstream consumers — including the plain CLI, below — have something to read after the engine call returns.
- **Plain-mode completion (single-shot, non-interactive CLI):** **`tddy-coder`**'s plain-mode entry points (**`run_full_workflow_plain`**, **`run_goal_plain`**) distinguish a genuine clarification pause from **`FreePromptingRecipe`**'s "**`Continue`** with no successor" pause by checking whether **`context["pending_questions"]`** is present and non-empty. When it isn't (the common case for a single **`prompting`** turn with a backend like **`stub`** or **`fastcontext`** that never asks a question), the plain CLI treats the turn as complete: it prints **`context["output"]`** via the recipe's **`plain_goal_cli_output`** and exits **`0`**, instead of erroring with `no pending questions`. This makes a single, non-interactive **`free-prompting`** turn (e.g. `tddy-coder --agent fastcontext --recipe free-prompting --prompt "..."`, piped or scripted) usable outside the TUI; continuing the conversation from the plain CLI across multiple turns is not supported — that remains a TUI/web (presenter-driven) capability.
...
- **Plain CLI:** A single non-interactive turn (e.g. `--agent fastcontext --recipe free-prompting --prompt "..."`) completes and prints the response instead of erroring, even though the backend never calls **`tddy-tools submit`** or asks a clarification question — see **FreePromptingRecipe** above for the mechanism.
