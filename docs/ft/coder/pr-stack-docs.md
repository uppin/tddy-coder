...
`pr_status.phase` deliberately records a draft as `open` (`tddy-pr-stack/src/stack_ops/adopt.rs:180`).
Opening a PR as a
...
(`tddy-pr-stack/src/stack_ops/nodes.rs:20-25`). A helper resolves the pair of paths and reports which exist; if a node
