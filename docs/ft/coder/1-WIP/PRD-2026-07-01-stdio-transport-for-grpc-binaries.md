...
- [ ] `tddy-coder` and `tddy-demo` gain a `--stdio` flag, mutually exclusive with `--grpc`/`--grpc-socket`/`--grpc-uds` and with TUI/plain mode, that serves the existing remote-control `RpcService` surface over a `tddy_stdio::StdioEndpoint` instead of a `tonic::transport::Server`. Any local view rendering under `--stdio` goes through `CapturingWriter::headless()`, never physical fd 1.
