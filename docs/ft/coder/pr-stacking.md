...
Three writers append a node — `seed_stack_with_base_session`, `add_planned_pr_node` and `adopt_pr_as_stack_node` — and they all go through two private helpers, `read_stack(dir, op)` (the changeset's stack, or an empty one) and `append_node_atomic(dir, node, op)` (the `update_stack_atomic` write, with `assign_missing_display_order` as the closure's first statement and `next_display_order` as the node's position, both decided inside the closure). Ordering is a stack invariant rather than each writer's business: a fourth writer that spelled the closure out and forgot `assign_missing_display_order` would hand its node a position an older node already holds. Each writer still spells its `StackNode` literal out in full, so a new field cannot be silently defaulted into one of them.

### Seeding the stack from an existing session *(added 2026-08-13)*

A stack can be **seeded at creation time** from a session the operator already has open. A new `pr-stack` orchestrator names one existing session as the stack's bottom node and comes up with that session's branch already in the plan, so "I have a session on `feat/auth-store` and I want the next PR to stack on top of it" needs no LLM round trip. The form control is [Base the stack on](#new-session-screen-recipe-dropdown-and-parent-picker); the wire field is `StartSessionRequest.pr_stack_base_session_id` (field 31), which the daemon threads through `SpawnOptions` as `--stack-seed-base-session <id>` — the same path `--stack-parent` already takes. The orchestrator's own `tddy-coder` process does the writing (`tddy_pr_stack::stack_ops::seed_stack_with_base_session`), because the session that owns a `changeset.yaml` is the process that writes it.

This is the fourth way a stack gets populated and the only one that starts from work that already exists. `analyze-stack` → `write-stack-plan` and chat refinement both ask the agent to plan from a feature description; `AddPlannedPr` appends a **planned** node (`branch: None`, `session_id: None`), which cannot describe a branch somebody is already working on; and `pr_adopt` resolves every fact it records from a **pull request** and refuses a branch that has none — while a session that has not opened a PR yet is precisely the session an operator wants to stack onto. `adopt_pr_as_stack_node` is therefore deliberately not extended to cover this case.

The seeded stack is a **single root node**:

| Field | Value |
|---|---|
| `node_id` | `n1` (the next free id in an empty stack) |
| `title` | the base session's `Changeset.name`, falling back to its branch when that is absent or blank — a node with no legible title is the one thing the panel cannot render usefully |
| `branch` | the base session's `Changeset.branch` — real from the start, so the node satisfies the spawn gate immediately |
| `branch_suggestion` | `None` — the node already owns a branch, and nothing here is going to choose a name |
| `session_id` | the base session's id |
| `parents` | `[]` — nothing existed for the bottom node to be stacked on |
| `pr_status` | `None` — whether that branch has a PR is the live-status poll's to discover, not the form's to assert; recording `planned` for a branch that may already have an open PR would misreport it until the first tick |
| `display_order` | assigned by `append_node_atomic` — `0` in the empty stack it is written into |

**No new stacking logic is involved.** A planned node parented on the seeded root bases its child worktree off `origin/<base session's branch>` through the path that already exists: `pr_stack_node_for_spawn` matches the node, `resolve_chain_base_ref` calls `Stack::base_ref_for_spawn`, which walks `parents` and returns the nearest non-merged ancestor's branch, and `link_stack_node_to_child_session` records the child's branch and session back onto the node.

**A seeded orchestrator starts in `StackDocsWritten`**, so its start goal is `orchestrate` rather than `analyze-stack`. *(It started in `StackPlanned` until the [per-PR documents](pr-stack-docs.md) landed; that state now routes to the `write-stack-docs` pass, and a seeded node — bound to a session whose work already exists — is precisely the node a retroactive PRD would document a decision nobody is about to make for. `pr_stack::STATE_STACK_DOCS_WRITTEN` names the new string.)* That is forced, not preferred: the seeded node owns a branch, and `reseed_stack_from_plan_if_unspawned` refuses a plan once any node owns a branch or a session — so a seeded session allowed to run its planning phase would have its `write-stack-plan` submit **rejected**. `pr_stack::STATE_STACK_PLANNED` names that persisted string once, beside `PrStackRecipe` in `tddy-workflow-recipes`; `tddy-coder` is what writes the state, because the coder owns workflow state and the recipe owns its state names. An unseeded orchestrator's start goal is unchanged.

**The base session's half of the refusals is one predicate**, `tddy_pr_stack::stack_ops::check_stack_seed_base`, which answers either with a `SeededBase { branch, title, repo_path }` or with a `StackSeedBaseRefusal` — an enum rather than a string, so the daemon maps it onto a status code (`Unresolvable` → `INVALID_ARGUMENT`, `Unusable` → `FAILED_PRECONDITION`) without re-deriving the rule that produced it, while its `Display` gives the two `Result<_, String>` callers a reason to interpolate. Three callers have to decide the same question and only one of them writes, so the rules — and their wording — exist once:

| Refused base session | Why |
|---|---|
| does not resolve to a readable changeset | there is nothing to bind a node to |
| records no `branch` | the branch is the node's whole purpose, and a branchless root fails the spawn gate for every descendant |
| carries an `orchestrator_session_id` | it is already a node of another orchestrator's stack, and two orchestrators with repoint and pull authority over one branch is ambiguous ownership |

The three caller-specific rules stay with their callers, each because it needs something `check_stack_seed_base` cannot see: **the base is the orchestrator itself** (`check_stack_seed_not_self`, comparing canonicalized directories so a symlinked sessions root cannot slip past it) — a stack cannot contain the session that owns it; **the stack already has nodes** — seeding is a creation-time act, and a second seed would add a duplicate root and reparent nothing (growing a stack is `add_planned_pr_node`'s job); and **the base works in another repository**, which only the RPC knows, since only it knows which project was asked for.

**Repository scoping.** `validate_stack_seed_base_session` compares **canonicalized** repository roots, and the relation is *at or under*, not equality: `Changeset.repo_path` records the project's main repo for a `tddy-coder` session but the session's own worktree (`<repo>/.worktrees/<name>`) for a claude-cli / cursor-cli / workspace session, and both work in the project's repository. A base session recording **no** `repo_path` is refused — "could not tell" is not "same repository". Project ids are deliberately not compared: an id is registry-local and not stable across hosts. This is not a narrower nice-to-have, because every descendant's worktree is created off `origin/<base branch>` in the *orchestrator's* project: offering a foreign branch means offering work that cannot be stacked, and the failure would land as a git error on the first descendant of an orchestrator that already looked seeded.

**The refusal is checked twice, on purpose.** The daemon refuses *before* anything spawns, so the form can show the reason in its error strip — a refusal raised after the spawn is invisible to a form that has already navigated away (see [connection-service.md § Stack-seed base session](../../../packages/tddy-daemon/docs/connection-service.md#stack-seed-base-session-on-startsession)). The seeding path refuses again as its own writer contract, since `--stack-seed-base-session` is reachable without the RPC. Same doubled-gate shape as `require_pr_stack_orchestrator` sitting beside each mutator's own validation.

**A refused seed writes nothing at all.** `tddy-coder`'s startup gate (`seed_pr_stack_from_base_session_if_requested`) checks the preconditions *before* `ensure_changeset_recipe` creates the `changeset.yaml` a tool session does not have yet — otherwise a refused start would leave a directory holding `{recipe: pr-stack, state: Init}`, and resuming it would bring up an ordinary **unseeded** orchestrator that nothing downstream can distinguish from a stack the agent has yet to plan. A base session named beside a recipe that owns no stack is refused for the same reason it is not merely ignored: silently dropping the id creates a session that looks like it was seeded and is not. A seeding failure fails session start outright — there is no fallback to an unseeded orchestrator, which is a different session from the one the operator asked for and would look successful. Restarts are unaffected: `resume_session` passes no base session, so the "stack already has nodes" refusal can never fail the resume of a seeded orchestrator; the flag reaches a coder only on the creating spawn.

**One base session, and no ordering over several.** Exactly one session can seed a stack, and the cap lives in the writer rather than only in the form, because it is a safety property. A linear order over several pre-existing branches would declare dependencies their git history does not have, and honouring it would mean rebasing branches an operator is already working in; with a single base node there is nothing to order. Multi-session seeding with operator-chosen ordering is logged in `docs/dev/TODO.md`.
...
A third flag belongs to the **orchestrator** rather than to a child: `--stack-seed-base-session <base-session-id>` seeds the new orchestrator's own stack with that session as its bottom node, and is consulted once at startup (`seed_pr_stack_from_base_session_if_requested`) — see [Seeding the stack from an existing session](#seeding-the-stack-from-an-existing-session-added-2026-08-13).

### Transport-agnostic spawn helper

`spawn_chain_child_worktree(sessions_root, parent_session_id, child_session_dir, child_repo, explicit_base)` lives in `packages/tddy-session-worktree/src/session_chain.rs`. It lifts the resolve-and-integrate logic that was previously embedded in `telegram_session_control.rs::merge_chain_integration_base_with_explicit_operator_overrides`, which becomes a thin wrapper. The orchestrator recipe calls this helper with the node's derived effective base ref, which may point at a sibling session's branch rather than the orchestrator's own branch.
...
- **Pipeline:** `analyze-stack` → `write-stack-plan` → `write-stack-docs` → `orchestrate` (terminal interactive loop)
  ...
  - `write-stack-plan` — the agent emits the plan with **one** call: `tddy-tools submit --goal write-stack-plan --data-stdin`. The JSON body is validated against the registered **`write-stack-plan`** goal schema (`urn:tddy:goal/write-stack-plan`; required `goal`, `version`, `prs`, each PR requiring `node_id` and `title`), so a wrong-shaped payload is refused with the offending paths instead of being accepted and dropped. `pr-stack-plan.md` is **derived** from that submission by `after_write_stack_plan`, not submitted separately. Seeding `Changeset.stack` from the written plan happens here / on entry to `orchestrate` (idempotent — same guard as the old `seed_orchestrator_stack_from_plan`).
...
- **Artifacts (manifest):** union of the plan and orchestrate artifacts — `stack_plan → stack-plan.yaml`, `stack_plan_md → pr-stack-plan.md`, `stack_status_md → stack-status.md`, `stack_status_json → stack-status.json`, and `exploration → exploration.md`. **All five live under `session_dir/artifacts/`.** The first two were written to the session *root* until the [per-PR documents](pr-stack-docs.md#artifact-path-correction) landed, which made them invisible to every wire reader (`context_docs_for_session`, `read_session_context_doc_utf8`, and `SESSION_ARTIFACT` host-document resolution all root at `artifacts/`) and unattachable to a child. `build_context_header` still falls back to the session root, so orchestrators created before the move keep working (the code-discovery map, see [Exploration artifact](exploration-artifact.md)).
...
- **State table:** `Init | AnalyzeStack → analyze-stack`; `WriteStackPlan → write-stack-plan`; `StackPlanned → write-stack-docs`; `StackDocsWritten → orchestrate` (drops into the interactive loop — **not** a terminal "Completed" state); `orchestrate → orchestrate` (pauses for input each turn); `failed → None`. `next_goal_for_state_with_changeset` still disambiguates a legacy `"Init"` with a populated `Changeset.stack` by resuming into `orchestrate` (previously `assess`). `status_for_state`: `StackPlanned | StackDocsWritten | orchestrate → "Active"`, `failed → "Failed"`, else `"Active"`. **Note the catch-all**: `next_goal_for_state` ends by routing every unrecognised state to `orchestrate`, which is what lets a legacy persisted state (`assess`, `wait`) resume rather than dead-end — so `StackDocsWritten` would reach `orchestrate` even if it were dropped from the table. `StackPlanned → write-stack-docs` is the entry the catch-all cannot supply. See [PR-stack documents](pr-stack-docs.md#the-write-stack-docs-goal).
...
This contract is **advisory, not machine-enforced.** It is carried by the `analyze-stack` and `write-stack-plan` system prompts, and appears in **both** deliberately: `write-stack-plan` is the goal `plan_refinement_goal()` returns, so it re-runs on every chat-driven refinement — a rule present only in `analyze-stack` would be silently dropped the first time an operator refined the plan. That copy also tells the agent a refinement request must not talk it into a layer-split stack. Pinned by `pr_boundary_scoping_rule_tests` in `pr_stack/hooks.rs`, which drives the real `before_task` seam rather than asserting on the string constants.

Both prompts live **once**, in `plan_pr_stack/prompt.rs`, parameterised by recipe name and shared by `PrStackRecipe` and the superseded `PlanPrStackRecipe`. Until 2026-08-29 each recipe carried its own copy, and the superseded one had drifted: it named a `submit` **tool** that has never existed (`advertised_tools` excludes the CLI subcommands) and asked for a second `stack-plan-md` submission nothing consumes. A single source is what keeps the boundary contract in both, rather than a note asking the next reader to port it across.
...
| `pr_add_planned` | Append a planned PR node mid-flow. Additive only — it never touches an existing node; editing one is `pr_update_planned`. | `stack_ops::add_planned_pr_node` |
...
| `pr_update_planned` | Edit a node's `title` / `description` (any time, including once it owns a branch, a session and an open PR) and its `branch_suggestion` (only while it owns no branch). Opt-in `sync_pr` also publishes the new title/body to the node's PR. | `stack_ops::{update_planned_pr_node, sync_node_to_github_pr}` |
| `pr_delete_planned` | Remove a node, reparenting its children onto that node's parents. Refuses a node whose PR is open. Branch, worktree and child session are left alone and reported back as unowned. | `stack_ops::delete_planned_pr_node` |
| `pr_set_parents` | Give a node a whole new parent list — the plan-level move, and the only reorder primitive (stack order is derived from `parents`). When the node owns a branch it is realigned exactly as a repoint realigns it. | `stack_ops::set_stack_node_parents` |
...
| `pr_adopt` | Create a node from an existing PR, bound to its head branch and PR reference. | `stack_ops::adopt_pr_into_stack` |

`stack_ops`, `assess` (`assemble_views`, `effective_base_ref`, `decide_next_action`) and `pr_insight` are modules of the [`tddy-pr-stack`](../../../packages/tddy-pr-stack/docs/architecture.md) crate; `execute_stack_merge` / `execute_stack_repoint` are the recipe-side bridge in `tddy_workflow_recipes::orchestrate_pr_stack`.
...
- **Stack base picker** *(added 2026-08-13)* — an optional **"Base the stack on"** `<select>` (`create-session-pr-stack-base-session-select`), rendered when `sessionType === "tool"` and the recipe is `pr-stack`. It hangs off the **recipe**, not the branch mode: the form creates only the orchestrator, whose branch mode describes the orchestrator's own worktree, while the base session is a property of the **stack** — so there is no branch mode for the control to qualify. Options come from `stackBaseSessionCandidates(sessions, { projectId, daemonInstanceId })` (`packages/tddy-web/src/utils/stackParents.ts`, a pure function over the same one `ListSessions` fetch the parent picker reads) — the sessions that own a branch, are in the form's effective project, are on its effective host, and are not already a node of another orchestrator's stack. An unresolved project offers **nothing**, since matching `""` would offer every session whose project is equally unknown. Each option reads `<session id> — <branch>`, the branch being what the seeded node is bound to and what every descendant is based on; the default option is empty and reads "None (agent plans the stack)", which is the pre-existing behaviour. The chosen id is sent as `pr_stack_base_session_id` on the tool branch only, and only while the recipe is still `pr-stack` — the daemon *refuses* a base session named beside another recipe, so a value chosen before switching recipes must not be submitted (the same shape as the existing `managedCodebase ? selectedSubagents : []`). Switching project or host clears the choice, so the control always shows what submit would do instead of holding a value the picker no longer offers behind a blank `<select>`. What the seeded stack then looks like: [Seeding the stack from an existing session](#seeding-the-stack-from-an-existing-session-added-2026-08-13).
...
> **Updated 2026-08-01 — the row is a control surface.** A row now collapses to a summary and expands to its full detail; the spawned indicator opens the child session it is bound to; row order is a **persisted `StackNode.display_order`** with move up/down rather than being re-derived from `parents` (so a merge or a repoint no longer moves a row the operator is reading); each row states how its branch stands against its base, computed non-mutatingly on the `QueryBranch` poll; and a row cleanly behind its base can **pull the base in** (merge by default, rebase on request). Note this makes `has-conflicts` a **live** fact rather than only an agent-set `internal_status` — the derivation table in `internal_status.rs` still never produces it, and the pull path deliberately does **not** stamp it, since a persisted copy could only go stale and clearing it would risk stomping an agent's `source: "override"`. See [PR-Stack live status § Panel UX](pr-stack-live-status.md#panel-ux-expandable-rows-session-links-persisted-order-base-sync-added-2026-08-01).
...
- **RPC:** `pr_stack.PrStackService.AddPlannedPr(AddPlannedPrRequest) -> AddPlannedPrResponse` (`pr_stack.proto`). Request carries `session_id` (the orchestrator), `title`, `description`, `branch_suggestion`, `parents` (chosen ancestor node ids), and `child_recipe`. Response carries `stack_plan_json` — the same wire shape as `SessionEntry.stack_plan_json` (field 23) — so the web reuses the existing `parseStackPlan` parser rather than a second message schema, plus `node_id` (field 2), the id of the node this call appended (see [Add and start in one step](#add-and-start-in-one-step-added-2026-08-13)).
...
Implementation: `tddy_pr_stack::stack_ops::add_planned_pr_node` (pure function — read/validate/append/write); daemon handler `ConnectionServiceImpl::add_planned_pr` in `connection_service.rs`.

#### Add and start in one step *(added 2026-08-13)*

The "+ New planned PR" form carries two submits — **"Add planned PR"** (`pr-stack-add-planned-pr-submit-btn`) and **"Add & start session"** (`pr-stack-add-planned-pr-start-btn`). The second opens the row's own [Start session dialog](../web/session-drawer.md#start-session-cta) for the node it just appended, instead of making the operator find the new row in the list and click its CTA. Nothing new happens on the host: the same `AddPlannedPr` call, then the same dialog, the same initial-values derivation (`baseBranchChoice`, `deriveStackBaseBranch`, the stack-parent wiring) and the same child-created callback the CTA already uses.

**The server names the node it created** — `AddPlannedPrResponse.node_id`, populated from the `StackNode` the daemon's handler already has in hand. The client cannot infer it by diffing the returned plan against the ids it held before the call: the orchestrator **agent** appends nodes to the same stack, and the screen's copy of the plan only refreshes on the next session-list refetch, so one response can carry several ids the screen has never seen — some appended before the operator's node and some after (one race while the form was open, one between the daemon's write and the re-read that serializes the response). Every positional pick among them is wrong in one of those cases, and the dialog it opens is pre-filled with somebody else's branch and title — which is then spawned against. A malformed plan made it worse still: `parseStackPlan` degrading to `{nodes: []}` made *every* node look new and selected the stack's bottom node, while the defensive "no new node" refusal never fired, because a node had been found. The field is additive — a client reading only `stack_plan_json` is unaffected — and the refusal is kept for the one case that is now a real inconsistency: a response whose plan does not carry the node it named leaves the list updated (the node really was appended server-side) and withholds only the session start, which the form reports in its error strip.
...
- [PR-stack documents](pr-stack-docs.md) — the per-PR PRD and changeset, the `write-stack-docs` goal, and how both spawn paths attach them to the child session
