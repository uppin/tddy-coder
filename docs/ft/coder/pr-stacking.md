...
**A seeded orchestrator starts in `StackPlanned`**, so its start goal is `orchestrate` rather than `analyze-stack`. That is forced, not preferred: the seeded node owns a branch, and `reseed_stack_from_plan_if_unspawned` refuses a plan once any node owns a branch or a session — so a seeded session allowed to run its planning phase would have its `write-stack-plan` submit **rejected**. `pr_stack::STATE_STACK_PLANNED` names that persisted string once, in the crate that owns the stack; `tddy-coder` is what writes the state, because the coder owns workflow state and the recipes crate owns the stack. An unseeded orchestrator's start goal is unchanged.
