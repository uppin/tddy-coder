...
**A seeded orchestrator starts in `StackDocsWritten`**, so its start goal is `orchestrate` rather than `analyze-stack`. *(It started in `StackPlanned` until the [per-PR documents](pr-stack-docs.md) landed; that state now routes to the `write-stack-docs` pass, and a seeded node — bound to a session whose work already exists — is precisely the node a retroactive PRD would document a decision nobody is about to make for. `pr_stack::STATE_STACK_DOCS_WRITTEN` names the new string.)* That is forced, not preferred: the seeded node owns a branch, and `reseed_stack_from_plan_if_unspawned` refuses a plan once any node owns a branch or a session — so a seeded session allowed to run its planning phase would have its `write-stack-plan` submit **rejected**. `pr_stack::STATE_STACK_PLANNED` names that persisted string once, in the crate that owns the stack; `tddy-coder` is what writes the state, because the coder owns workflow state and the recipes crate owns the stack. An unseeded orchestrator's start goal is unchanged.

**The base session's half of the refusals is one predicate**, `pr_stack::check_stack_seed_base`, which answers either with a `SeededBase { branch, title, repo_path }` or with a `StackSeedBaseRefusal` — an enum rather than a string, so the daemon maps it onto a status code (`Unresolvable` → `INVALID_ARGUMENT`, `Unusable` → `FAILED_PRECONDITION`) without re-deriving the rule that produced it, while its `Display` gives the two `Result<_, String>` callers a reason to interpolate. Three callers have to decide the same question and only one of them writes, so the rules — and their wording — exist once:
