...
The Implementation Step is the TDD Red-Green phase of the tddy-coder workflow. The **red** goal reads PRD.md and acceptance-tests.md from the plan directory, creates skeleton production code and failing lower-level tests, and persists its session ID to `.impl-session`. The **green** goal resumes the red session via `.impl-session`, reads progress.md (required) plus PRD.md and acceptance-tests.md (optional for context), implements production-grade code to make all failing tests pass, updates progress.md and acceptance-tests.md with results, and verifies completion by running both unit and acceptance tests.

## Background

tddy-coder follows a strict TDD workflow: plan → acceptance-tests → red → green. The red and green goals form the implementation phase. Red creates skeletons and failing tests; green implements production code to make them pass. Both use the same Claude session (red starts it, green resumes it) for context continuity.
...
4. `--model`, `--agent-output`, `--allowed-tools`, `--debug` work with both goals
...
7. Write `.impl-session` with the session ID for green to resume
...
3. Read the session ID from `.impl-session` in the plan directory (persisted by the red goal)
...
11. On successful exit, output a human-readable summary (tests passed/failed count, implementation summary)
...
- Written by the red goal to the plan directory
- Contains the session ID used by the red goal's Claude session
- Green goal reads this file to resume the same session for context continuity
...
- [x] Red goal persists session ID to `.impl-session` in the plan directory
...
- [x] Green goal resumes the red session via `.impl-session` file using `--resume`
...
- [x] Error handling: missing plan-dir, missing progress.md, missing .impl-session
