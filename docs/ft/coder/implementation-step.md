...
5. Parse Claude's output to extract summary, tests, and skeletons (validated against `schemas/red.schema.json` before deserialization; on failure, session resumes with validation errors, 1 retry)
...
7. Parse Claude's output to extract summary, test results (pass/fail per test), and implementation details (validated against `schemas/green.schema.json` before deserialization; on failure, session resumes with validation errors, 1 retry)
