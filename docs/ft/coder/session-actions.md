...
- **Backend:** The **`acceptance-tests`** goal uses the session’s selected **`CodingBackend`** and the same model resolution path as other structured goals; there is no separate YAML or CLI agent override for authoring **`actions/`**. The recipe hook supplies the dedicated **`acceptance_tests::system_prompt`** and assembled user **`prompt`** for that invocation (distinct prompts from other goals, same backend).
