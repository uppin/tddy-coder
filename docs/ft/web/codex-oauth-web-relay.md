...
End-to-end capture requires **`BROWSER`** (or equivalent) routing to Tddy, daemon **RPC or event delivery** to the correct **tddy-web** session, and delivery of the callback to the running Codex CLI listener. With **tddy-desktop (Electrobun)** and an embedded or reachable **`tddy-daemon`** using **`livekit.common_room`**, the operator browser still hits **`http://127.0.0.1:{port}/auth/callback`** on the **operator** machine, but **`tddy-daemon`** (**`oauth_loopback_tunnel`**) accepts that TCP and **tunnels raw bytes** over **LiveKit** via **`loopback_tunnel.LoopbackTunnelService.StreamBytes`** to **`127.0.0.1:{port}`** on the **session host**; Codex sees the same HTTP callback as a purely local workflow. The session-side **`LoopbackTunnelService`** rejects privileged ports (**`open_port`** at least **1024**) and requires a well-formed first **`TunnelChunk`**. See **[tddy-desktop-electrobun.md](../desktop/tddy-desktop-electrobun.md)** and **[Codex OAuth relay (daemon)](../daemon/codex-oauth-relay.md)** (operator tunnel section). Other integrations live outside this document; see **[`codex-oauth-relay.md`](../../../packages/tddy-daemon-auth/docs/codex-oauth-relay.md)** for API detail.
