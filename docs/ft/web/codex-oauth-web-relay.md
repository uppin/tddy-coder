...
End-to-end capture requires **`BROWSER`** (or equivalent) routing to Tddy, daemon **RPC or event delivery** to the correct **tddy-web** session, and delivery of the callback to the running Codex CLI listener. With **tddy-desktop (Electrobun)**, the operator browser still hits **`http://127.0.0.1:{port}/auth/callback`** on the **operator** machine; the desktop process **tunnels raw TCP** over **LiveKit** via **`loopback_tunnel.LoopbackTunnelService.StreamBytes`** to **`127.0.0.1:{port}`** on the **session host**, so Codex sees the same HTTP callback as in a purely local workflow. The session-side **`LoopbackTunnelService`** implementation rejects privileged ports (**`open_port`** must be at least **1024**) and requires a well-formed first **`TunnelChunk`**. See **[tddy-desktop-electrobun.md](../desktop/tddy-desktop-electrobun.md)**. Other integrations live outside this document; see **[Codex OAuth relay (daemon)](../daemon/codex-oauth-relay.md)** and **[`codex-oauth-relay.md`](../../../packages/tddy-daemon/docs/codex-oauth-relay.md)** for technical detail.
