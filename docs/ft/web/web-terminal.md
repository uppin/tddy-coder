...
- **Touch scrolling**: A **one-finger vertical drag** scrolls; dragging **down** pulls earlier output into view. Who owns the gesture is decided at `touchstart`, the same three ways the wheel is on desktop: in the **alternate screen** (DEC 1049 — a full-screen TUI such as the Claude CLI) the drag is handed to the terminal as wheel notches at the touch point, so a mouse-tracking TUI receives SGR wheel reports (button 64/65) and a pager receives ghostty-web's Up/Down emulation — the application scrolls its own content, and the lazy-history double buffer is never engaged; in the **normal screen** the drag scrolls the emulator's own scrollback. See [Terminal replay — lazy scroll](terminal-replay-lazy-scroll.md).
