...
- **Font zoom (pitch)**: The terminal surface includes **pitch-in** (larger glyphs), **pitch-out** (smaller glyphs), and **reset** to the session baseline. Controls live on **`TerminalZoomToolbar`** (`data-testid` **`terminal-zoom-pitch-in`**, **`terminal-zoom-pitch-out`**, **`terminal-zoom-reset`**). Default font bounds are **8–32** px with step **1**; at the minimum or maximum, the corresponding control is disabled. **`GhosttyTerminal`** exposes the live size on **`data-terminal-font-size`** (integer string). Font changes apply to the running ghostty-web **`Terminal`** (`options.fontSize`), then **`FitAddon.fit()`** recomputes columns and rows; **`onResize`** runs when the grid changes, so the existing resize OSC sequence reaches the TUI backend on the same input path as keyboard data. **`ConnectionTerminalChrome`** mounts the toolbar and passes **`terminalBaselineFontSize`** (default **14**) so reset matches the initial **`GhosttyTerminal`** **`fontSize`**. **`GhosttyTerminalLiveKit`** accepts **`fontSize`** (default **14**) and passes it to **`GhosttyTerminal`** and to the toolbar when **`connectionOverlay`** is absent; when **`connectionOverlay`** is set, chrome alone supplies the toolbar so controls are not duplicated. Zoom actions use window **`CustomEvent`** names **`tddy-terminal-zoom`** and **`tddy-terminal-font-size-sync`**; payloads are validated before handling. With focus inside **`[data-testid='ghostty-terminal']`** or **`[data-testid='terminal-zoom-toolbar']`**, **Ctrl** or **⌘** with **+**/**=** (pitch-in), **-** (pitch-out), or **0** (reset) performs the same actions as the buttons. Optional trace logging for bridge and font application uses **`VITE_TERMINAL_ZOOM_DEBUG=true`** in the Vite build, or **`debugLogging`** on **`GhosttyTerminal`** for component-level traces.
