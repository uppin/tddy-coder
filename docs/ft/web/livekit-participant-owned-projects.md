...
- **`session`** (published by `tddy-coder` participants, and *(2026-08-30)* by `tddy-daemon` for a `claude-cli` session, which previously published none): an object carrying the running session's presence — `workflow_goal`, `workflow_state`, `elapsed_display`, `agent`, `model`, `activity_status`, `recipe`, `repo_path`, `pending_elicitation` — plus its **stack association**, `session_id`, `orchestrator_session_id`, `stack_node_id` and `branch`. The web's sessions drawer overlays it onto sessions-list rows so active cross-host rows show goal/state/agent/model with no `ListSessions` fan-out, and the PR-Stack view joins on the association to find a planned PR's child running on another host — presence is the only signal that crosses a host boundary, since `ListSessions` answers for one daemon's own sessions tree. `stack_node_id` lives **only** here and is deliberately not a `SessionEntry` field: it is needed exactly where a participant is live. Because the merge is shallow and there are now two publishers, the block's shape is owned by `tddy_core::session_participant_metadata` and every key is always emitted, empty where the publisher has nothing to say. See [Session Drawer Screen § Sessions list metadata from participants](session-drawer.md#sessions-list-metadata-from-participants), [Session Participant RPC & Metadata](../coder/session-participant-rpc.md) and [PR-Stack live status § Cross-host planned PRs](../coder/pr-stack-live-status.md).
