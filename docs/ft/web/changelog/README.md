...
Each changelog entry is **its own file**, named `YYYY-MM-DD-<slug>.md`.
...
layout replaces (it used to be a single `changelog.md` with
`## YYYY-MM-DD — Title` entries, merged with git's `union` driver, which still stranded
