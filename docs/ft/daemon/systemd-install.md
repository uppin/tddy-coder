...
**`daemon.yaml`** carries the matching **`supervisor.socket_path`**. ⚠️ **That block is not consulted for spawning yet:** no daemon code path routes session/clone/sandbox spawning through the supervisor (see *Known incomplete* in **`docs/dev/1-WIP/CS-2026-08-02-tddy-supervisor.md`**), so today every session is still spawned by the daemon's own forked worker as the daemon user. The template declares it anyway, and says so inline, so the host is already configured when that wiring lands — at which point an unreachable or refusing supervisor **fails the operation** rather than falling back to spawning as the daemon user.
