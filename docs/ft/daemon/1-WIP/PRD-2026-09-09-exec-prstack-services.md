...
Per the policy node 7 set, every family that moves stays reachable on the daemon's local Unix socket.
All three of this node's services go on it — 4 + 4 + 8 = **16 adapter methods**, generated by node 6's
`generate_tonic_adapter`, which moves this node from wave 3 to wave 4.

`exec_tools.ExecToolService` is the consequential one: `ExecuteTool` is what
`tddy-sandbox-runner`'s relay allowlist gates and what an agent inside a jail calls, so a family that
silently left the socket would fail there rather than anywhere a developer is looking.
...
- [ ] all three services answer over the **local Unix socket**, not only over Connect-HTTP
- [ ] all three adapters are **generated**; this node adds no hand-written `*_tonic_adapter.rs`
