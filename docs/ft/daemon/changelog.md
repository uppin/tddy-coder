...
## 2026-04-05 — Telegram: document body on review (requirements)

- **Requirement**: When the presenter shows a document for **review / approve / reject**, operators must receive the **full document text on Telegram** (split across messages when needed), followed by **Approve**, **Reject**, and **Refine** inline actions aligned with the Virtual TUI.
- **Formatting**: Markdown or styled spans must follow Telegram **[message entities](https://core.telegram.org/api/entities)** rules (offsets and lengths in **UTF-16 code units**; supplementary-plane code points count as two units).
- **Feature docs**: [telegram-notifications.md](telegram-notifications.md) (Presenter stream: elicitation), [telegram-session-control.md](telegram-session-control.md) (Document review on Telegram).
