# Initial discovery — `#carve` 7/9 `telegram`
...
## Exploration 4 — node-specific: the cluster is genuinely mutual, and the destination cannot be `tddy-telegram`

Run while writing this node's PRD, to answer two questions the plan depended on: does this node
really need `#carve` 3/9, and can the modules go into the `tddy-telegram` crate that already exists?

### Sequence

1. `grep -E '^tddy-' tddy-telegram/Cargo.toml` and `ls tddy-telegram/src/` — the existing crate's
   dependency set and surface.
2. Production-only `crate::telegram*` census of each of the six moving modules, comments stripped.

### Grep — notable hits

**`tddy-telegram` today** — six modules (`active_elicitation`, `elicitation`, `sender`,
`telegram_github_link`, `telegram_tracked_session`, `lib`) and five `tddy-*` dependencies:
`tddy-rpc`, `tddy-core`, `tddy-service`, `tddy-github`, `tddy-daemon-kernel`. **Not
`tddy-session-lifecycle`** — which depends on *it*.

**Production references among the six moving modules:**

```
telegram_bot                     -> telegram_notifier  telegram_session_control  telegram_tracked_session
telegram_notifier                -> telegram_multi_select_shortcuts  telegram_session_control  telegram_tracked_session
telegram_session_control         -> telegram_github_link  telegram_multi_select_shortcuts
                                    telegram_notifier  telegram_session_subscriber  telegram_tracked_session
telegram_multi_select_shortcuts  -> telegram_notifier
telegram_session_subscriber      -> telegram_notifier
session_notification_subscribers -> telegram_session_subscriber
```

### Findings

**This is the one node in the stack with a genuinely mutual cluster.** Two pairs reference each other
in production code:

| Pair | Direction A | Direction B |
|---|---|---|
| `telegram_notifier` ↔ `telegram_session_control` | notifier names control | control names notifier |
| `telegram_notifier` ↔ `telegram_multi_select_shortcuts` | notifier names shortcuts | shortcuts names notifier |

The leaf-first trick that lets `#carve` 5/9 and 6/9 skip cluster support **cannot work here**: there
is no ordering in which every rewrite is correct when made, because moving either member of a mutual
pair rewrites the other before it has moved. **`#carve` 3/9 is a real behaviour dependency for this
node**, and it is the node that justifies 3/9 existing.

Note that `telegram_session_control → telegram_bot` appears in a naive grep but is **test-only**;
production has `telegram_bot → telegram_session_control` one way. It does not change the verdict, as
the two pairs above are mutual regardless.

**The destination must be a new crate.** `tddy-telegram` does not depend on `tddy-session-lifecycle`,
and `tddy-session-lifecycle` depends on `tddy-telegram`. The moving modules need
`cli_session_manager`, `session_deletion`, `session_list_enrichment`, `cursor_cli_spawn`,
`session_reader`, `project_storage`, `branch_owner` and `presenter_intent_client` — all in
`tddy-session-lifecycle`. Putting them in `tddy-telegram` would make it depend on the crate that
depends on it. So: **`tddy-telegram-control`, depending on both.**

**The back-edge remains exactly one field and one call site** (Exploration 2), and
`TelegramDaemonHooks` is defined in `telegram_session_subscriber.rs` — one of the moving modules. So
the port inversion is not optional dressing; without it the cluster cannot leave at all.

**What moves untested.** `telegram_bot.rs` holds the crate's two worst functions by CRAP score —
`telegram_callback_handler` (complexity 88) and `telegram_message_handler` (52), both never executed
by any test. They relocate **unchanged**. Decomposing or testing them inside a move would make the
diff unreviewable and put behaviour risk into a node whose whole claim is that it has none.
