# Initial discovery — optional-livekit node `connection-model`
...
## Exploration 2 — node-specific: the call sites this node migrates

`useSelectedDaemon().room` readers, classified. This node owns the first group only.

| File:line | Use | Owner |
|---|---|---|
| `src/rpc/selectedDaemon.tsx:323-324` | `useDaemonClientFor` -> `useLiveKitClient(service, room, daemonRpcIdentity(id))` | **n1** |
| `src/rpc/useHostFanOut.ts:108` | `const { room, daemons } = useSelectedDaemon()` — fan-out client per host | **n1** |
| `src/components/models/useModelRegistryFanOut.ts:207` | same shape, model registry fan-out | **n1** |
| `src/components/models/ModelChatDialog.tsx:38` | peer client for the chat dialog | **n1** |
| `src/components/projects/ProjectsAppPage.tsx:104` | peer client for project registry | **n1** |
| `src/components/sessions/SessionsDrawerScreen.tsx:93,102-109` | cross-host `ConnectionService` client built at click time via `liveKitFactory(room, daemonRpcIdentity(id))` | **n1** |
| `src/components/sessions/SessionsDrawerScreen.tsx:257-284` | session-client fallback + `onRoom` capture | n3 |
| `src/components/sessions/SessionsDrawerScreen.tsx:315-317,856` | `useRoomParticipants(room)` — cross-host session presence | n2 accessor / n4 gating |
| `src/components/livekit/LiveKitAppPage.tsx:21-22` | presence panel | n2 accessor / n4 gating |
| `src/rpc-playground/RpcPlaygroundAppPage.tsx:81-82` | presence list | n2 accessor / n4 gating |

`SessionsDrawerScreen.tsx` is the one file three nodes touch, in three disjoint regions. Keeping the
regions disjoint is why n1 migrates *all* RPC uses rather than only the ones in its own screens.

### The existing seams this node builds on

- `RpcTransportProvider` already carries `liveKitFactoryIsOverridden` — a test-double detector that
  exists precisely because the production factory needs a real connected `Room`. The provider
  registry replaces the need for that flag at daemon level; it stays for media until node 4.
- `wrapTransportWithAuthGate` (`src/rpc/authGatedTransport.ts`) wraps every peer transport with the
  request-time-fresh token gate. A provider must keep that wrap, or a peer connection silently sends
  stale credentials.
- `TrafficMeterRegistry` meters per room name (`registry.get(room.name || "livekit")`). A
  transport-neutral provider needs a meter key that is not a room name — the host id is the natural
  one, and the control plane already uses a literal key (`"http"`) for the same reason.
