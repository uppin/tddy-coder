...
- [x] `tddy-task`: `IdleTimeoutTracker` lifted from `tddy-daemon::relay_idle` into `tddy-task` (+ unit tests); the daemon re-exports it for the stable `relay_idle` path, and the LSP registry now tracks per-server idle with it.
...
- [x] `tddy-sandbox-app`: register the executor before spawn (`main.rs::run_macos`) + set `TDDY_LSP_TOOLS` in the jail env (`spawn.rs`) when `is_available(&params.repo)`
...
- [x] End-to-end test (`tddy-lsp-executor/tests/e2e_test.rs`): two Rust targets in one workspace, driven through the real `TddyLspExecutor` → registry → LSP client against the fake server, share a single reused server task and resolve references/definition. (Executor-level e2e; a live daemon-process e2e over gRPC/jail remains an optional heavier check.)
- [x] `LspClient` replies to server→client requests (`client/registerCapability`, `window/workDoneProgress/create` → null; `workspace/configuration` → one null per item), so real rust-analyzer never stalls waiting on us.
