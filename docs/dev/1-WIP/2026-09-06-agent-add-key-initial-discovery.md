# Initial discovery — `agent-add-key` (#hosts-screen 6/8)

**Node:** load a key into a host's ssh-agent with an end-to-end encrypted passphrase
...
## Exploration 5 — node-specific: the prompt channel, the crypto, and the hardening to invert

### The chosen channel shape, and why the teardown rule bites here

Decision (whole-work discovery § Design decisions): **server-stream + unary reply**, not the ACP bidi
shape. Precedent: `StreamWorktreeStats` + `CalculateWorktreeSize` — a stream that reports state and a
unary call that acts on it (`connection.proto:80-81`, hook
`packages/tddy-web/src/rpc/useWorktreeStatsStream.ts`).

⚠ **`packages/tddy-codegen/docs/server-streaming.md` — the obligation this node cannot avoid.** A
handler whose stream can be **silent** must `tokio::select!` on `tx.closed()` as well as breaking on a
send error; otherwise the handler task leaks forever, one per subscription.

`stream_host_stats` escapes this only because it emits unconditionally every 5 s. **A prompt stream is
silent almost all the time** — that is its normal state. It is precisely the leaking case.

Worked example to copy: `pump_rooms` in `packages/tddy-daemon/src/livekit_rooms_stream.rs`. Pinned
regression test to mirror: `packages/tddy-daemon/tests/stream_livekit_rooms_rpc.rs`
`::stops_reading_the_server_once_the_subscriber_is_gone`.

### The hardening this node has to invert, behind an opt-in

`packages/tddy-core/src/worktree.rs:39-52`:

```rust
/// … hardens the process against interactive hangs: stdin is closed and
...
`packages/tddy-daemon/src/config.rs:192-200` documents today's workaround — an `ssh_command` using
`BatchMode=yes` with the macOS Keychain.

**But note what the node 5 decision buys us**: because the daemon speaks the **agent protocol directly**
and decrypts the private key **in-process** with the supplied passphrase, it never runs `ssh-add` and
never needs a TTY or `SSH_ASKPASS`. **The git hardening may not need inverting at all** — the passphrase
never reaches a subprocess. Confirm this during the red phase; if it holds, this node's blast radius is
substantially smaller than the whole-work discovery first assumed.

### Crypto: shapes that exist, and what has to be new

**Browser side — no new dependency.** `SubtleCrypto` (`crypto.subtle.importKey` / `encrypt` with
`RSA-OAEP`) is a platform API. A passphrase is small: 2048-bit RSA-OAEP-SHA256 carries ~190 bytes of
plaintext, comfortably more than any passphrase, so **no hybrid AEAD envelope is needed**.

**Daemon side — a third new crate.** Nodes 5 added an agent client and a key crate; an RSA
implementation is additional. It must be pinned and justified.

**Precedent for a transport-restricted method:** `mint_local_token`
(`packages/tddy-daemon/src/connection_service.rs:~16481`) — the shape to follow if the reply RPC is to
refuse untrusted transports.

**Precedent for an encrypted secret at rest:** `packages/tddy-daemon/src/screen_sharing_vault.rs`
(`DerivedKey`, `ScreenSharingVault`). Its *storage* model is explicitly **not** wanted here — the
decision is forward-and-drop, never persist — but its key-derivation code is worth reading before
writing new crypto.

### ⚠ The guarantee, stated honestly

`docs/ft/web/projects-screen-multi-host.md` § Trust model: *"any participant that can join the LiveKit
common room appears as an eligible daemon and can receive forwarded RPCs (including the caller's
`session_token`). The common room is treated as a trusted peer group, **not a cryptographically
authenticated one**."*

Encryption under the host's public key defeats a **passive** relay: the room and any forwarding daemon
stop seeing plaintext. It does **not**, by itself, defeat an **active** peer, because the client learns
that public key over the same unauthenticated channel — a hostile participant advertising itself as
`daemon-<instance_id>` can publish its own key and decrypt what the browser sends.

Three ways to close or bound that, one of which the PRD must commit to:

1. **Key continuity (TOFU)** — pin a host's public key on first sight, warn loudly on change. SSH's own
   model, and the one an operator already understands.
2. **Out-of-band verification** — display a fingerprint the operator compares against the host.
3. **Accept and disclose** — protect against passive observation only, and say so in the dialog.

### What the UI already does that is *not* this

`ScreenSharingPassphraseDialog.tsx` / `VncPassphraseDialog.tsx`
(`packages/tddy-web/src/components/sessions/`) are the **opposite direction**: the *UI* decides to ask
before calling a unary. Useful as a dialog-shape reference, not as a mechanism — nothing there is
server-initiated.

The only genuine server-asks-UI precedent is the ACP bidi flow
(`packages/tddy-web/src/components/chat/useAcpSession.ts:129`, question at `:381`, reply at `:479`),
which was considered and **not** chosen.

### Testing the round trip

The Cypress in-memory backend routes by `methodKind`, so a server-streaming fake is an async generator
(`cypress/support/rpc/connectionServiceBackend.ts:566-586`) and must end with
`await new Promise<never>(() => undefined)` to stay open. A prompt-then-answer round trip is expressible:
the fake yields a prompt, the component answers via the unary, and the fake's **unary** handler records
it — and unary calls **are** recorded by the interceptor (`backend.ts:147-157`), so `callsTo(...)`
works for the answer even though the stream is not recorded.

⚠ **The test must assert the answer is not plaintext.** A round-trip test that only checks the key was
added would pass just as well with the passphrase in the clear — which is the whole thing this node
exists to prevent.
