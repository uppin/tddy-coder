# Initial discovery — `host-identity` (#hosts-screen 4/8)

**Node:** each host's git identity and GitHub CLI status
...
## Exploration 5 — node-specific: probing a host for git and `gh`

### This node introduces the first *host capability probe*

Nodes 1–3 report facts the daemon already holds (membership, telemetry). This node is the first to ask
**"what is installed and configured on this machine"**, and it establishes the RPC shape that nodes 5
and 7 will extend. Getting the shape right here matters more than the two probes themselves.

### Why cross-host routing is free

`packages/tddy-daemon/src/connection_service.rs:9171-9183`:

```rust
async fn rpc_served_by_peer<Req, Resp>(...) -> ...
```

`Ok(None)` means "this daemon's own to serve"; otherwise the call is relayed over the common room to
`daemon-{instance_id}`. `PeerRoute` (`livekit_peer_discovery.rs:202-213`) was "renamed … to reflect
that this routing logic is now applied to all eligible RPCs (StartSession, ExecuteTool,
ListExecTools, …)". `common_room_slot` (`:9160-9169`) fails with `failed_precondition` when
`livekit.common_room` is unconfigured.

→ **A new unary RPC on `ConnectionService` carrying a `daemon_instance_id` inherits peer routing with
  no transport work.** This is the single biggest reason this node is small.

### How to run the probes

`packages/tddy-daemon/src/spawner.rs:736-830`:

```rust
pub fn run_capture_as_user(os_user: &str, program: &Path, args: &[String]) -> anyhow::Result<String>
```

Unix only (`:829-834` is a non-Unix stub that bails). Does `getpwnam_r` → `pre_exec { setgid,
initgroups, setuid }`, `current_dir(home)`, sets `HOME` and `PATH = merge_spawn_child_path(None)`,
null stdin, piped stdout/stderr; a non-zero exit becomes an error carrying stderr.

**This is the right helper**, and the `HOME`/`PATH` handling is the reason: both probes are
user-scoped. `git config --get user.email` reads `$HOME/.gitconfig`, and `gh auth status` reads
`$HOME/.config/gh/hosts.yml`. Running as the daemon's own user would report the wrong identity.

⚠ **`ExecuteTool` is the wrong mechanism.** Every `ExecuteToolRequest` requires a `session_id`
(`connection.proto:1545-1551`), and a host probe has no session. There is no session-less
"run this on host X" RPC today, and this node must not invent a general one — a general remote-exec
RPC is a far larger security surface than two fixed probes.

### ⚠ The supervisor allowlist

`packages/tddy-supervisor/src/policy.rs:96` `resolve_tool_path(policy, requested)` and `:124`
`resolve_env(policy, requested)`. **Env is an allowlist, not a denylist** — any key not listed is
`SupervisorError::Denied`, and the test at `:487` uses `SSH_AUTH_SOCK` as the example of a denied key.

Under a supervisor the daemon runs as an unprivileged child declared in `supervisor.yaml` (CLAUDE.md).
Whether `git` and `gh` are resolvable tool paths, and whether the probe's env survives, must be
**verified during the red phase** rather than assumed. This is the most likely source of a
"works locally, fails on a real deployment" gap in this node.

### What "authenticated as whom" can mean, and the collision to avoid

`packages/tddy-github` is a **GitHub OAuth web-login** crate — `GitHubOAuthProvider` (`provider.rs:16-32`),
`GitHubUser { id, login, avatar_url, name }` (`:6-11`), `SessionTokenSigner`, and `GitHubTokenStore`
(`token_store.rs:15-25`) retaining the raw access token per login
(`packages/tddy-daemon/src/github_token_store.rs:38-73`). A **second** model exists: `GITHUB_TOKEN` /
`GH_TOKEN` env + `curl` REST (`packages/tddy-workflow-recipes/src/github_rest_common.rs:19-34`,
`packages/tddy-tools/src/github_pr.rs`).

⚠ **Three unrelated GitHub identities can disagree**: who tddy's web session is logged in as, whose
token is in the env, and who `gh` on that host is authenticated as. This node reports **only the
third**, and the UI must label it as the host's `gh` login so it is not mistaken for the tddy user
shown in `UserAvatar.tsx`.

### Naming: do not reuse "capability"

`ConnectionCapability = "rpc" | "media" | "presence"` (`packages/tddy-web/src/rpc/connections/types.ts:33`)
means **what the wire carries**, and `useCapabilityAvailability` gates surfaces on it. The sense this
node introduces — what software a host has — is unrelated. Name it distinctly (e.g. *host tooling*)
or two different meanings collide across nodes 4–8 in the same components.

### Absence is a first-class answer

`gh` not installed, `gh` installed but logged out, and `gh` present but the probe failed are **three
different facts**, and so are "no git identity configured" versus "the probe could not run". Collapsing
them into an empty string would be a fabricated value. The response type must distinguish them, and the
tests must pin each one.
