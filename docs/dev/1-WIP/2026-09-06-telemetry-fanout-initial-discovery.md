# Initial discovery — `telemetry-fanout` (#hosts-screen 2/8)

**Node:** live CPU and disk on every host row
...
## Exploration 5 — node-specific: streaming one feed per host

### What exists to generalize

`packages/tddy-web/src/rpc/useHostStats.ts:38` — the whole hook:
...
`useDaemonClient(Service)` resolves **the selected daemon** —
`packages/tddy-web/src/rpc/selectedDaemon.tsx:394-409`, delegating to `useHostClient`. The per-host
client this node needs is already exposed directly:

`packages/tddy-web/src/rpc/connections/registry.tsx:207` — `useHostClient(service, hostId)`,
and `:195` `useHostConnection(hostId)` for the connection's `status` and `capabilities`.

→ The generalization is `useHostClient(ConnectionService, hostId)` in place of `useDaemonClient(...)`.
  No transport work; the registry already resolves a client per host.

### The constraint this node has to design around

**A row without a connection cannot stream.** `connectHost(hostId)` returns `HostConnection | null`
(`rpc/connections/types.ts:121-134`) — `null` when no registered provider can reach that host, which
is exactly the case for an offline host from node 1's registry. `useHostClient` therefore returns
`null`, and `useHostStats` today would simply never subscribe.

So the row states are three, not two:

| Registry says | Connection | Telemetry cell |
|---|---|---|
| online | connected | live values |
| online | connecting / error | a pending or error state, **not** fabricated zeros |
| offline | none | `—`, no subscription attempted |

⚠ **Do not fabricate a value for a host that is not reporting.** CLAUDE.md forbids fallbacks; a zeroed
CPU bar for an unreachable host is a lie an operator will act on.

### Cost: N concurrent streams

Each subscription is a live server-streaming RPC emitting **every 5 s** (`connection.proto:255-261`).
A fleet of 20 hosts means 20 permanently open streams and 20 events every 5 s. Two mitigations to
weigh in this node's design, both recorded in the changeset's Decisions section:

1. **Subscribe only for hosts the registry reports online** — offline rows cost nothing.
2. **Subscribe only while the Hosts screen is mounted** — the hook tears down on unmount, which the
   existing `cancelled`-flag pattern already does correctly.

### The teardown obligation does NOT bite here

`packages/tddy-codegen/docs/server-streaming.md` requires a `tokio::select!` on `tx.closed()` only for
a handler whose stream can be **silent**. `stream_host_stats` emits unconditionally every 5 s, so its
existing `if tx.send(...).is_err() { break; }` is sufficient. **This node adds no daemon-side stream**,
so it inherits no new teardown obligation. (Node 6 does.)

### Testing: streaming fakes and the counter workaround

`anInMemoryRpcBackend`'s recording interceptor **only records unary calls**
(`packages/tddy-connectrpc-testkit/src/backend.ts:147-157`). Streaming assertions therefore need a
closure counter exposed on the backend — the existing precedent is
`packages/tddy-web/cypress/support/rpc/connectionServiceBackend.ts:296-298` and `:637`
(`hostStatsStreamCount()`).

This node needs that counter **per host**, since the property under test is "one stream per online
host, none for offline hosts". The existing single counter is not sufficient and must be widened to a
per-`hostId` tally in the shared backend helper.

⚠ `mountWithRpc` gives **one** transport for every host (`cypress/support/rpc/inMemory.tsx:39` —
the LiveKit factory "ignores room/identity and returns the same in-memory transport"). So a component
test cannot distinguish two hosts by transport; the fake must key its behaviour off the **request**,
and `withSelectedDaemon(...)` (`cypress/support/rpc/withSelectedDaemon.tsx:86`) supplies the host list.

→ **`StreamHostStatsRequest` carries only `session_token`** (`connection.proto:2245-2248`). There is
  no host discriminator in the payload, because today it is always addressed to the selected daemon
  over its own connection. A per-host component test therefore cannot tell the two subscriptions
  apart from inside the fake. Resolving this is part of this node's red phase — the likely answer is
  a per-host counter keyed on the transport the registry handed out, or a test-visible hook seam.
