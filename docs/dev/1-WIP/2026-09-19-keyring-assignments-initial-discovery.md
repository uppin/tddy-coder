# Initial discovery — `#keyring` 5/9 `assignments`

**Node**: `#keyring` 5/9 — assignments · branch `feature/keyring/assignments` · base `feature/keyring/accounts (#511)`
**Changeset**: [2026-09-19-keyring-assignments.md](./2026-09-19-keyring-assignments.md)
...
## Exploration 2 — the project registry row and the RPC shape to copy — 2026-09-19

**Question**: what does a project row look like today, how was the last optional field added, and
what does a daemon-minted `AccountId` mean on a host that did not mint it?

### `ProjectData` — the row

`packages/tddy-projects/src/project_storage.rs:10`, stored in `~/.tddy/projects/projects.yaml`:
...
`ProjectsFile { projects: Vec<ProjectData> }`; `read_projects` returns an empty vec for a missing
file; `write_projects` rewrites the whole list through `tddy_core::atomic_file::write_atomic`.

**Three optional fields, all added the same way** — `#[serde(default, skip_serializing_if = …)]`
plus a documented behaviour when absent. Adding a fourth is a solved problem, and an older daemon
reading a newer row is already the established expectation.

### The RPC precedent

`packages/tddy-service/proto/project.proto:14-26` — `ProjectService` has five RPCs. The one to copy
is `SetProjectDefaultBranch`, whose comment states the scope explicitly:

> Set a project's default integration base ref (logical-project scope: forwarded to peer hosts
> owning the same project_id).

Its request is `{ session_token, project_id, main_branch_ref, daemon_instance_id }` and its response
returns the updated `ProjectEntry`. `SetProjectAccounts` is that shape with a repeated field, so the
forwarding path already exists and does not have to be designed.

`ProjectEntry` has seven fields, `default_remote = 7`; `accounts` takes tag 8.

### The state that has no name yet

An `AccountId` is **daemon-minted** (3/9) and lives in that daemon's vault. A logical-project
assignment is forwarded to every host owning the `project_id`. So a peer receives an id naming a
record it does not have, and will not have until 6/9 propagates the vault.

This is a **real steady state for the whole of wave 4**, not a transient. Two ways to report it are
wrong:

- as `NotAssigned` — the person made a choice and would see it absent on another host, unexplained;
- as an error — nothing failed; the assignment is intact and will resolve once the vault syncs.

Hence a fourth resolver answer, `UnknownOnThisHost(AccountId)`, which is honest about both halves:
assigned, and not resolvable here yet.

### Why the resolver is not in `tddy-projects`

Answering `UnknownOnThisHost` requires reading this host's vault. Putting that in `tddy-projects`
would place `tddy-credentials` on the dependency path of every consumer of projects — branches,
checkout paths, worktree provisioning — for a question almost none of them asks.
`tddy-accounts` (4/9) already depends on the store, so the resolver goes there: `tddy-projects`
**stores** the assignment, `tddy-accounts` **interprets** it.

### The fallback that would pass every other test

The dangerous shape here is the helpful one: a project with no assignment, a vault holding exactly
one GitHub account, and a resolver that returns it. Every "does the happy path work" test passes, and
the first commit pushed under the wrong identity is the first symptom.

The developer's instruction is explicit — *"if github account is not assigned, the backend wouldn't
be able to resolve it to use those credentials"* — so the no-fallback test is written as an assertion
about what the resolver does **not** do: given exactly one candidate in the vault and no assignment,
the answer is `NotAssigned`.

### What this node deliberately does not touch

Every git and GitHub call site. The resolver's real consumer is 9/9, which is last in the line and
blocked on [#492](https://github.com/uppin/tddy-coder/pull/492) by the developer's instruction to
plan git/github changes as late as possible. This node's own observable behaviour is the assignment
and its three rendered states — which is what keeps it a vertical slice rather than a surface waiting
for a consumer.
