# Initial discovery — `desktop-connect` (#hosts-screen 8/8)

**Node:** open a host's desktop in the in-app viewer
...
## Exploration 5 — node-specific: re-scoping an existing viewer from session to host

### The viewer is not greenfield — this is the single most important fact for sizing

`packages/tddy-web/src/components/sessions/ScreenSharingOverlay.tsx:1-35` — a full-screen overlay that
subscribes to the bridge participant's LiveKit **`VideoTrack`**
(`room.remoteParticipants.get(bridgeIdentity)`) and renders it into a `<video>`. Props:
`{ room, bridgeIdentity, trackName, width, height, onClose }`.

Siblings: `VncOverlay.tsx` (legacy), `SessionScreenSharingTab.tsx`, `SessionVncTab.tsx`,
`ScreenSharingPassphraseDialog.tsx`, `screenSharingTabState.ts`, `vncInput.ts` (input forwarding).

⚠ **There is no browser-side VNC/RDP protocol client, and there must not be one.** `package.json` has
no novnc / guacamole / rfb. Rendering is *always* a LiveKit video track produced by a daemon-side
bridge. Node 8 therefore **reuses this overlay**; it does not implement a protocol.

### What genuinely has to be new: host scope

`packages/tddy-service/proto/screen_sharing.proto` — every request carries `session_token` **and
`session_id`** (`ListTargetsRequest:38-41` and the rest), and `screen_sharing_vault.rs` keeps the
encrypted credential vault **under the session directory**.

`StartStreamResponse:100-107` returns `{ livekit_room, livekit_url, bridge_identity, track_name, width,
height }` — exactly the props the overlay needs. So the *response* shape already fits a host-scoped
call; it is the *addressing and storage* that are session-bound.

Spawning: `screen_sharing_service.rs:163-176` — `Command::new(&binary).stdin(piped)`, config as JSON on
**stdin** ("to avoid exposing credentials in argv/ps"), PID tracked in `active_bridges` for
`stop_stream`.

→ Node 8 owns: a **host-scoped target model**, a **host-scoped vault**, and host-scoped
  start/stop. It does **not** own the bridge, the overlay, or the input channel.

### ⚠ Media capability gates the whole feature

`packages/tddy-web/src/components/sessions/InspectorTabs.tsx:17,101-110` — the VNC / Screen Sharing tabs
are **removed** (not disabled) when the connection lacks the `media` capability.

`IPC_CAPABILITIES = new Set(["rpc"])` (`packages/tddy-web/src/rpc/connections/localHost.ts:59`) — **a
frame pipe carries no media**. So on the desktop build's own IPC-reached host, a LiveKit video track
cannot arrive at all.

→ The connect action must be gated on `media` via `useCapabilityAvailability`, and — consistent with the
  existing precedent — **removed rather than shown broken** where media is unavailable. This is a real
  functional limit worth stating in the PRD, not a detail.

### Credentials, and the vault decision this node must make

A remote desktop usually needs a password. The session-scoped precedent **stores** it encrypted
(`screen_sharing_vault.rs`, `UnlockVault` RPC, `ScreenSharingPassphraseDialog`).

⚠ **Node 6 deliberately chose never to persist a secret.** Node 8 must decide explicitly whether a
host-scoped desktop credential follows the existing *vault* precedent or node 6's *forward-and-drop*
posture — and if it persists one, say plainly why the two secrets are treated differently. Left
implicit, this is exactly the kind of inconsistency a reviewer should catch.

Node 6 also leaves a reusable asset: an established encrypted prompt channel. Reusing it for a desktop
password would be consistent and would avoid a second vault.
