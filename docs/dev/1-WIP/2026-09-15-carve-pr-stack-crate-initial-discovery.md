# Initial discovery — `#carve` 9/9 `pr-stack-crate`
...
## Exploration 4 — node-specific: finding a seam that does not close a cycle

Run while writing this node's PRD. The extraction is only valid if the moving set names nothing in
`tddy-workflow-recipes`, so this pass measured every `crate::` edge across the candidate seam.

### Sequence

1. Production-only `crate::` census of all eleven `pr_stack/` and `orchestrate_pr_stack/` modules.
2. `strip writer.rs | grep -oE 'crate::[a-z_]+'` — can `writer` move?
3. Per-file census of `plan_pr_stack/` — can it move?
4. `awk 'NR>418 && NR<=1967' pr_stack/mod.rs | grep -nE 'plan_pr_stack|crate::writer'` — exactly
   which stack operations reach recipe-side.

### Grep — notable hits

**The eleven modules' production `crate::` dependencies:**

```
pr_stack/mod.rs        -> orchestrate_pr_stack{,::git_ops,::github,::pr_insight,
                          ::pr_number_from_status_url}  plan_pr_stack{,::planned_prs_into_stack_nodes,
                          ::validate_stack_plan}  writer
pr_stack/hooks.rs      -> orchestrate_pr_stack  plan_pr_stack  writer::write_exploration_file
pr_stack/bridge.rs     -> orchestrate_pr_stack::{recover_in_flight_stack_op,
                          seed_orchestrator_stack_from_plan}  plan_pr_stack
pr_stack/docs.rs       -> (nothing)
orchestrate_pr_stack/assess.rs      -> (nothing)
orchestrate_pr_stack/git_ops.rs     -> (nothing)
orchestrate_pr_stack/pr_insight.rs  -> (nothing)
orchestrate_pr_stack/actions.rs     -> (nothing)
orchestrate_pr_stack/hooks.rs       -> (nothing)
orchestrate_pr_stack/mod.rs         -> (nothing)
orchestrate_pr_stack/bridge.rs      -> orchestrate_pr_stack::{git_ops,github,transient}
                                       plan_pr_stack{,::…}  pr_stack::assign_missing_display_order
```

**`writer.rs → crate::parser`** — so moving `writer` would drag the parsers.

**`plan_pr_stack/{mod,hooks}.rs → crate::pr_stack`** — mutual with `pr_stack`.
`plan_pr_stack/prompt.rs` is clean.

**Stack operations reaching recipe-side, after line 418 — only two places:**

| Offset from 418 | Reference |
|---:|---|
| +4 | `crate::plan_pr_stack::validate_stack_plan(plan)` |
| +27 | `crate::plan_pr_stack::planned_prs_into_stack_nodes(&plan.prs)` |

Both are inside `reseed_stack_from_plan_if_unspawned`. The two other hits at +229/+230 are **doc
comments**, not code.

### Findings

**`pr_stack ↔ orchestrate_pr_stack` is mutual**, so this node needs `#carve` 3/9's cluster support —
`pr_stack/mod.rs` names `orchestrate_pr_stack::{git_ops,github,pr_insight}` and
`orchestrate_pr_stack/bridge.rs` names `crate::pr_stack::assign_missing_display_order`. No ordering
makes both correct.

**The seam is decided by two snags, and both have a clean cut:**

1. **`reseed_stack_from_plan_if_unspawned` stays.** It is the *only* stack operation after 418 that
   reaches `plan_pr_stack`, and `plan_pr_stack` is mutually referenced with `pr_stack` — so moving it
   would pull a module that cannot come. Leaving this one function behind is what makes the rest of
   the 1,500 lines movable, and it is a plan→stack bridge, which is recipe-side by nature anyway.
2. **`crate::writer::EXPLORATION_BASENAME`** is two references to one `&str` const, and `writer.rs`
   depends on `crate::parser`. The const moves and `writer.rs` re-exports it, or it is duplicated
   with a comment naming the other definition. Decided at implementation; both keep the set clean.

**Six modules move with zero cross-seam edges of their own:** `pr_stack/docs.rs` and
`orchestrate_pr_stack/{assess,git_ops,pr_insight,actions}.rs` name nothing at all, and
`orchestrate_pr_stack/hooks.rs` and `mod.rs` are clean too — though the latter two are recipe glue
and stay.

**Phase ordering is load-bearing here in a way it is not elsewhere.** The stack operations and
`PrStackRecipe` are in **one file**, so there is no module for `move_module_to_crate` to take until
`extract_module` has cut the seam at line 418. Phase A is not a convenience; without it Phase C has
no anchor.
