# Initial discovery — `#carve` 8/9 `presenter-split`
...
## Exploration 4 — node-specific: the 46 methods, and the operation that cannot move them

Run while writing this node's PRD, to establish the method partition and to check which restructure
operation applies.

### Sequence

1. `awk 'NR>=152 && NR<=1788 { if ($0 ~ /^    (pub )?(async )?fn /) print }' presenter_impl.rs` —
   every method in the production impl, in declaration order.
2. Re-read the plan schema's three `extract_module` geometries.

### The 46 methods, in declaration order

```
 1 new                                   24 sync_select_highlight
 2 set_agent_activity_context            25 handle_intent
 3 with_broadcast                        26 clarification_answers_ready
 4 with_intent_sender                    27 send_clarification_answers
 5 with_recipe_resolver                  28 collect_answers
 6 with_worktree_dir                     29 advance_to_next_question
 7 connect_view                          30 flush_agent_output_buffer
 8 broadcast                             31 finalize_agent_line_in_activity_log
 9 agent_activity_head_commit            32 sync_agent_partial_activity_log
10 agent_activity_declared_paths         33 log_activity
11 capture_agent_activity                34 poll_tool_calls
12 broadcast_mode_changed                35 poll_workflow
13 prd_body_for_plan_review              36 start_workflow
14 approve_plan_from_review_or_viewer    37 restart_workflow
15 show_backend_selection                38 spawn_workflow
16 configure_deferred_workflow_start     39 changeset_read_dir
17 is_backend_selection_pending          40 finish_start_slash_structured_run_if_needed
18 broadcast_error_recovery              41 try_handle_start_slash_line
19 start_workflow_from_pending_if_any    42 state
20 apply_deferred_backend_factory        43 is_done
21 handle_backend_selection_answer       44 take_workflow_result
22 handle_recipe_slash_selection_answer  45 apply_feature_slash_builtin_recipe
23 select_highlight_matches              46 recipe_slash_selection_active
```

**They are not in cohesion order.** `broadcast` (8), `broadcast_mode_changed` (12) and
`broadcast_error_recovery` (18) are separated by activity and plan-review methods; the backend
selection group runs 15–22 but picks up 45–46 at the far end. So the partition is a **regrouping**,
not a set of contiguous cuts — which matters for Phase A, since it means six `impl` blocks cannot be
made by inserting five braces at existing boundaries; methods move between blocks.

### The operation finding — `extract_module` is refused here

The plan schema records three geometries for `extract_module` and marks exactly one as blocking:
...
This file is the third case, unambiguously: 46 methods in one `impl`, calling each other freely —
`poll_workflow` calls `broadcast` and `log_activity`; `handle_intent` calls `collect_answers`. The
schema says the refusal *"fires before the assist runs"* and that **no ordering fixes it**, because
an `impl` body cannot hold a `mod` — the sibling can be moved neither out of the way first nor after.

Its stated remedy is the one this node takes: *"Grow the seam to carry the whole `impl`."* Rust
permits a type's inherent methods to live in several `impl` blocks across modules of one crate, so
partitioning the single block into six **by hand** converts a refused operation into six instances of
the schema's **cheapest** move — *"Moving a whole `impl` is free of caller churn, and is the cheapest
restructuring move Rust has."*

### Findings

**This node is manual-first, and that is forced rather than chosen.** Phase A has no operation behind
it: there is no assist for "split this impl", and the partition is a regrouping rather than five
cuts. Phase B is then six of the cheapest operation available.

**The partition maps onto `#carve` 4/9's five sub-structs with three methods left over** — `state`,
`is_done`, `take_workflow_result` — which read `state` and `workflow_result` directly and stay on the
root, alongside the seven wiring constructors that touch every group by definition.

**AC4 is a hypothesis test, not a gate.** The five-way field grouping is 4/9's claim about cohesion,
and this node is the first place it meets the methods. A group that straddles two sub-structs is
evidence about the grouping, so the criterion asks for it to be **named** rather than absent.
