# Initial discovery — `#keyring` 6/9 `sync`

**Node**: `#keyring` 6/9 — sync · branch `feature/keyring/sync` · base `feature/keyring/assignments (#512)`
**Changeset**: [2026-09-19-keyring-sync.md](./2026-09-19-keyring-sync.md)
...
## Exploration 2 — the existing peer trust model, and what it cannot be asked to carry — 2026-09-19

**Question**: what authorises a peer today, is that enough to hand it a person's GitHub credential,
and where would a sync engine attach?

### The trust model, quoted rather than paraphrased
...
> receive a forwarded `StartSession` carrying the full RPC body, including `session_token`. Operators
> rely on a private LiveKit project, restricted network access, and trusted hosts — there is no
> separate cryptographic attestation that a participant runs `tddy-daemon`.

Two things follow:

1. **It is honest, and it is adequate for what it covers.** An operator picks a host from a list and
   a session start is forwarded to it.
2. **It cannot carry credential propagation.** Anything holding the room's LiveKit credentials would
   receive a person's GitHub token. `#keyring` 1/9 exists precisely because this stack stopped
   treating the LiveKit API secret as an authorisation decision — reusing room membership here would
   reintroduce that in a worse place.

The developer's constraint matches: *"only peers that have a shared secret (in most cases it's going
to server-side daemons)"*.

**Conclusion**: this node needs a gate of its own, and must say explicitly that it does **not**
retrofit that gate onto `StartSession` forwarding — that is a separate change with a separate blast
radius.

### What 1/9 leaves available

A per-daemon Ed25519 identity and a `KeyDirectory` port (implemented in `tddy-daemon-livekit`,
because `dependency_boundary_unit.rs` forbids `tddy-daemon-auth` on that crate's dependency path).

That gives an **authenticated sender**. It does not give **authorisation** — "that daemon may hold my
credentials" has a per-deployment answer that no signature can supply. Hence two independent checks,
and the two tests that matter are the ones proving neither is sufficient alone.

### Why a separate transport key

The signing key could technically be converted for key agreement. Not doing that costs one more
published key and buys two things: key uses are not mixed, and a transport key can be rotated without
invalidating every session token the daemon has signed. It is published through 1/9's `KeyDirectory`
and signed by the identity key, so a peer knows whose transport key it is.

⚠ Needs `x25519-dalek` — CLAUDE.md § ASK, requested in the green phase, not assumed here.

### Where the engine lives

`packages/tddy-daemon-livekit/src/` is five modules: `common_room_supervisor.rs`,
`livekit_peer_discovery.rs`, `livekit_rooms_stream.rs`, `livekit_service.rs`, `session_room.rs`.
Writing the sync engine into them would be the shortest path and would repeat exactly what
`heavy-dependency-livekit-peer-forwarding` measures: one misplaced dependency, 14 dependents, 6
paying for something they never use.

So: `tddy-credential-sync` owns a `PeerTransport` port, `tddy-daemon-livekit` implements it. The
practical consequence is that authorisation, wrapping and reconciliation are testable against an
in-memory fake peer that can be made to fail exactly one check — only the adapter and the join
trigger need a real LiveKit server.

### The resurrection problem

LWW over `(provider, account)` by `updated_at` handles concurrent edits. It does **not** handle
deletion: a record removed here and still present on a peer comes back on the next sync, and the
person's removal silently undoes itself. Deletions therefore become **tombstones** carrying the same
key and timestamp shape, winning or losing by the same rule.

This is the one place where a "simplest thing that works" sync would be wrong in a way nobody notices
until a revoked credential is back in use.

### What must never travel

The vault's data key, its KEK, and the login credential that derives it. A recipient re-seals what it
receives under its own vault key, so two daemons share a **record**, never a **key** — compromising
one does not unwrap the other's disk. The test asserts this over the serialised outbound bytes rather
than over the struct, so a field added later fails rather than passing quietly.
