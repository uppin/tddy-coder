# Initial Discovery: Index daemon plan store

**Changeset**: [2026-09-26-index-daemon-plan-store.md](./2026-09-26-index-daemon-plan-store.md)
**Date**: 2026-09-26
...
1. Read `tddy-index-daemon/src/apply.rs` (per-run parse, `StatePaths::under`)
2. Read `index.rs` (per-root queue, no plan state)
3. Read `serve.rs:182` / `main.rs:104` (shutdown hooks)
4. Read `tddy-tools/src/index_client.rs:33` (daemon routing when `TDDY_INDEX_SOCKET` set)
5. Read `tddy-code-restructuring/src/restructure_cli.rs` (in-process front end), `runner.rs` (`StatePaths::{under, for_plan}`, `open_run_after`), `journal.rs` (ops by index)
6. Read code issue `stale-repo-scoped-restructure-state-apply.md`
7. Read `packages/tddy-tools/tests/` — `restructure_cli_acceptance.rs`, `index_daemon_client_acceptance.rs` exist
