# Initial discovery — `#keyring` 3/9 `store`

**Node**: `#keyring` 3/9 — store · branch `feature/keyring/store` · base `feature/keyring/desktop-login (#509)`
**Changeset**: [2026-09-19-keyring-store.md](./2026-09-19-keyring-store.md)
...
## Exploration 2 — the store surface being replaced, and the encryption pattern to reuse — 2026-09-19

**Question**: what exactly does `GitHubTokenStore` cover, who reads it, and is there an existing
encrypted-at-rest pattern in this workspace worth reusing rather than inventing?

### The trait and its one implementation

- `packages/tddy-github/src/token_store.rs` — two methods, `put(login, access_token)` and
  `get(login)`. Its doc comment carries **two standing rules** that outlive the file:
  1. the GitHub token is deliberately **not** in the session token, because that token reaches a
     browser over a plain-http LAN origin;
  2. a failed `put` **fails the login** — "a session minted without its token is a half-login".
  The same comment calls the session token "HMAC", which `#keyring` 1/9 makes false.
- `packages/tddy-daemon-auth/src/github_token_store.rs` — the only implementation.
  `HashMap<String, String>` → JSON → `write_atomic_with_mode(path, 0o600)`. Concurrent logins are
  serialised by a process-wide `static PUT_LOCK: Mutex<()>`; without it two logins clobber, because
  the file is rewritten whole.

### Every consumer

- `packages/tddy-daemon-auth/src/auth.rs:83-152` — constructs it, threads it into
  `auth_service_entry`, returns it in `AuthBuildResult`.
- `packages/tddy-daemon/src/runtime.rs:882` — constructs the file-backed one and injects it.
- `packages/tddy-session-lifecycle/src/connection_service/svc_pr_status_for_caller.rs:93` — **the
  only read outside the auth crate**:
  ```rust
  let stored = self.github_token_store.as_ref().and_then(|store| store.get(github_login));
  ...
  ```
  Three outcomes — `Empty`, `Unavailable(reason)`, `Perform(token)` — and an acceptance test must
  pin all three across the migration.

**Conclusion**: the blast radius is one trait, one implementation, one external reader and two
construction sites. Replacing it outright is cheaper than extending a 2-argument signature that has
no room for a provider, an account identity or a refresh token.

### The encryption pattern already in the tree
...
`write_atomic_with_mode(…, 0o600)`. Sound in its primitives.

**Four limits this node fixes rather than copies:**

| Limit | Why it matters |
|---|---|
| Item metadata sits **outside** the AEAD | A label or provider can be tampered with undetected |
| Argon2 parameters are **unversioned** on disk | Changing a cost parameter presents as a failed decrypt, indistinguishable from a wrong key |
| The passphrase travels **on the wire** in an RPC | The secret that opens the vault is handled by the transport |
| `DerivedKey` is cached **un-zeroized** in a `HashMap` keyed by `session_id` | Key material outlives the session in process memory |

### Dependencies already in the workspace

`chacha20poly1305 0.10`, `argon2 0.5`, `sha2 0.10`, `hmac 0.12`, `subtle 2.6`, `rand`. HKDF-Extract
and -Expand are a dozen lines over `hmac` + `sha2`, so **no new external dependency is required**.
`hkdf 0.12` and `zeroize` would be tidier and both need CLAUDE.md § ASK approval; the design does
not depend on either.

### The one question this exploration had to answer

**How does a login produce a deterministic key with no daemon-held master key?**

`KEK = HKDF-SHA256(ikm = the GitHub access token, salt = per-vault random, info = "tddy-credential-vault-v1/" || <github user id>)`,
which unwraps a random 32-byte data key. The disk holds
`{ header{kdf, params, salt}, wrapped_DK, verifier, records[] }` — ciphertext and a wrapped key, and
nothing that opens it.

This is deterministic **because `#keyring` 2/9 chose an OAuth App**, whose user access token does not
expire. Under a GitHub App — 8-hour user tokens — the input keying material changes every few hours
and this scheme fails; that would be a re-plan, not an adjustment.

**The accepted cost**: a user who revokes and re-approves gets a different token, so the KEK no
longer unwraps and the vault reports `Locked`. There is deliberately **no** second key and **no**
silent re-initialisation — either would restore exactly the property being removed. Mitigation: every
successful login re-wraps the data key, so a rotation observed while a session can still be
established costs nothing.
