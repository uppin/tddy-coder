# Initial discovery — `#keyring` 7/9 `screen-share`

**Node**: `#keyring` 7/9 — screen-share · branch `feature/keyring/screen-share` · base `feature/keyring/sync (#513)`
**Changeset**: [2026-09-19-keyring-screen-share.md](./2026-09-19-keyring-screen-share.md)
...
## Exploration 2 — what this node deletes, and what depends on it

**Scope**: `#keyring` 7/9 only. Read in full: `packages/tddy-screen-sharing/src/screen_sharing_vault.rs`,
`packages/tddy-screen-sharing/src/screen_sharing_service.rs`, `packages/tddy-service/proto/screen_sharing.proto`.

### 2.1 `screen_sharing_vault.rs` — 394 lines, all of them deleted

The public surface, as the file has it:

| Symbol | Line | Note |
|---|---|---|
| `VERIFIER_PLAINTEXT` | :39 | a known plaintext, sealed, to test a candidate key |
| `ScreenSharingTarget` | :43 | `id`, `label`, `host`, `port`, `protocol`, `username` — **plaintext on disk** |
| `DerivedKey(pub [u8; 32])` | :56 | doc comment: *"Held in daemon memory only; never written to disk"* |
| `ScreenSharingVault` | :204 | `create`, `unlock`, `exists`, `add_target`, `list_targets`, `remove_target`, `decrypt_password`, `is_passphrase_valid` |
| `VAULT_FILENAME` | :389 | `".screen-sharing.yaml"` |
| `vault_path(session_dir)` | :392 | **this is what makes targets per-session** |

The cryptography is sound and was the model for 3/9: ChaCha20-Poly1305, a fresh nonce per item, a
verifier ciphertext, `write_atomic_with_mode(…, 0o600)`. What 3/9 changed while copying it is the
four-row table in this node's PRD — and each row is a defect in *this* file, which is why deleting it
closes all four rather than leaving three behind.

`DerivedKey`'s doc comment is worth quoting because it is **true and insufficient**. The key is
genuinely never written to disk. It is also held in a `HashMap` for the session's lifetime with no
`Drop` that clears it, so it is in the process image for as long as the session lives. 3/9's
`SessionVault` zeroizes; this node is where screen-sharing inherits that.

### 2.2 `screen_sharing_service.rs` — the cache and its single reader

```rust
pub type ScreenSharingKeyCache = Arc<Mutex<HashMap<String, DerivedKey>>>;   // :39-42
```

`require_key` at :203 is the only reader: it looks the session up and errors when nothing is cached —
which is the error a person sees as "enter your passphrase". Both go with the vault.

The cache is constructed and wired in `packages/tddy-daemon/src/runtime.rs:1261`, inside the function
the `complexity-runtime-build` code-issue records at 806 lines. This node makes it shorter. It does
not fix it, which is why the verdict is ⚠ DURING and not ✅.

### 2.3 `screen_sharing.proto` — ten RPCs, one deleted

Six session-scoped RPCs and four host-scoped. The one that goes:

```proto
message UnlockVaultRequest {     // :98-102
...
with the comment: *"Called by the browser when the user enters the passphrase. The derived key is
cached in daemon memory for the session lifetime."*

The comment describes the design exactly, and the design is the thing being removed: `session_token`
already proves the person is present, so `passphrase` is a second proof of the same fact, typed by
hand and sent over a wire. Deleting the field is not enough — the message and the RPC go, because a
deprecated RPC carrying a passphrase is still an RPC carrying a passphrase.

**This is why the load-bearing test asserts over the generated descriptor** rather than over the
`.proto` text. A reintroduction — by a revert, by a merge, by a well-meant migration path — then
fails a test rather than passing review.

### 2.4 The scope change nobody asked for, and why it is here anyway

`vault_path(session_dir)` scopes targets to a session. The credential store is per user. Moving
screen-sharing into the store therefore *changes behaviour*: a target added in one session shows up
in the next.

That is not a change this node's brief demanded, and it is recorded as a decision in both the PRD and
the changeset with the rejected alternative stated (session id inside `metadata`, preserving today's
behaviour exactly). The argument for the change is that today's behaviour is the annoyance, not the
feature: "saved targets" that vanish with the session are saved in name only. The cost of reversing
is one metadata field, which is why the decision is recorded rather than escalated.

### 2.5 No migration, and why that is not a fallback question

Reading an existing `.screen-sharing.yaml` requires the passphrase that unlocked it. A migration path
therefore keeps the deleted RPC alive for one run — the exact shape the node exists to remove. The
developer's instruction on the whole stack is *"The change can be breaking, don't add fallbacks."*
Targets are re-added once.

### 2.6 What this node does **not** touch, verified by reading

`StartStream`, `StopStream`, the LiveKit bridge identity, track naming, the four host-target RPCs and
VNC input are all independent of where the password came from — the service resolves a password and
hands it to the stream. Changing the resolution changes one call site.

`tddy-credentials` is untouched. That is the finding this node exists to produce: if screen-sharing
had needed a change in the store, 3/9 would have built a GitHub token file with a longer name. The
review criterion is written into the changeset as an acceptance item so it is checked rather than
assumed.

### 2.7 Measurement status

`tddy-screen-sharing` and `tddy-service` have **no `docs/` directory**, so neither has ever been run
through `/analyze-code-issues`. The four limits above were measured by reading, during this stack's
Step 2. They are recorded in this node's changeset rather than as four new code-issue records,
because writing records for a file scheduled for deletion in the same stack is churn — and the
changeset is what carries them into the change history at wrap.

**"Not measured" is not "clean"** for the rest of either package. This node claims nothing about it.
