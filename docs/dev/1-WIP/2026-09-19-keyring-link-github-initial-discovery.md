# Initial discovery — `#keyring` 8/9 `link-github`

**Node**: `#keyring` 8/9 — link-github · branch `feature/keyring/link-github` · base `feature/keyring/screen-share (#514)`
**Changeset**: [2026-09-19-keyring-link-github.md](./2026-09-19-keyring-link-github.md)
...
## Exploration 2 — where identity and credential are welded together

**Scope**: `#keyring` 8/9 only. Read in full: `packages/tddy-service/proto/auth.proto`.

### 2.1 `AuthService` — five RPCs, and what each one *ends up* doing

```
GetAuthUrl      → authorize_url + state
ExchangeCode    → session_token + GitHubUser + refresh_token
GetAuthStatus   → authenticated + GitHubUser
RefreshSession  → session_token + GitHubUser + refresh_token   (sliding)
Logout          → ()
```

The weld is in `ExchangeCode`. One call does three things at once: it obtains a GitHub token, it
learns who the person is, and it **mints a session**. Nothing in the surface lets a caller ask for
the first two without the third.

That is correct for login and fatal for linking. A person adding a second account is already
authenticated; the flow they need ends with *a record exists*, not *you are now someone else*.

### 2.2 The split this node needs

2/9's device flow has the same shape as `ExchangeCode` — poll GitHub until the person authorises,
then hold a token. The split is:

| Half | Used by |
|---|---|
| obtain a GitHub access token + the `GitHubUser` it belongs to | login **and** link |
| establish a session from one — mint `session_token`, mint `refresh_token` | login **only** |

Login composes both. Linking calls the first alone and writes a record.

**This is the whole node, in one sentence**, and it is also the one thing an implementation will be
tempted to skip: the login path already exists, already works, and already returns a token that could
simply be ignored. The load-bearing test therefore asserts the caller's `session_token` and identity
are unchanged after a link, because the shortcut passes every positive test.

### 2.3 Why `account_id` stability is not a detail

`#keyring` 5/9 stores project → account assignments as `AccountId` strings, and `AccountId` is
**daemon-minted and opaque** — chosen in 4/9 precisely so it does not leak a provider's identifier
shape.

An opaque id has one obligation in exchange: it must be stable. If re-linking an account (the ordinary
thing a person does when a token is revoked or expires) mints a fresh id, then:

1. the record is correct,
2. the account lists correctly on the Accounts screen,
3. and every project pointing at the old id resolves to `NotAssigned`.

5/9's resolver reports that faithfully, and `NotAssigned` is indistinguishable from *never assigned*.
So the symptom is a person re-authorising an account and finding their projects quietly unassigned,
with nothing anywhere connecting the two events. Hence dedup on the GitHub **user id**, and a re-link
that updates in place.

The login name is the wrong key for a different reason: GitHub lets a name be released and
re-registered, so a record keyed on it can end up holding a stranger's token under a person's
project.

### 2.4 The account that cannot be removed

3/9 derives the vault KEK from the **login** account's access token. So the session's own account is
a record like any other, except that removing it makes every other record unreadable.

Two coherent designs:

| | Behaviour | Cost |
|---|---|---|
| **chosen** | `RemoveAccount` refuses, with the reason | a person who genuinely wants to stop using that account signs out instead |
| rejected | remove it and lock the vault | technically honest; leaves a person with a store they cannot open and no obvious route back |

The second is not *wrong* — it is what the cryptography actually does. It is rejected because the
resulting state is unexplainable at the moment it occurs. Recorded in the changeset so a reviewer can
overrule it.

### 2.5 What this node does **not** need

- **5/9 and 6/9 are not edges.** A linked account is assignable because it is a record, and syncs
  because it is a record. This node writes no code in either.
- **9/9 is the dependent**, not a co-requisite. Producing a second account is the whole deliverable;
  *using* one for git is the next node.
- **`tddy-credentials` is unchanged.** Same criterion as 7/9: a second way to acquire a credential
  should need nothing from the store.

### 2.6 Measurement status

`tddy-daemon-auth` has one code-issue record, `complexity-auth-build-auth-entries.md`, in the path of
the flow this node splits. **Not claimed** — 2/9 is the lowest node touching the same function, and
this node's two RPCs go on `AccountsService`, so it adds nothing to the recorded count.

`tddy-web` has no `docs/code-issues/` at all. **"Not measured" is not "clean"**; this node claims
nothing about it.
