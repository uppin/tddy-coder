# Initial discovery — `agent-keys` (#hosts-screen 5/8)

**Node:** each host's ssh-agent and the keys it holds
...
## Exploration 5 — node-specific: reaching an ssh-agent from the daemon

### Starting from nothing

`grep -n "name = .*ssh" Cargo.lock` → **zero matches**. No ssh-agent, ssh-key, russh or openssh crate
is in the dependency graph at all. Nothing in the workspace reads `SSH_AUTH_SOCK`.

The only three hits for the term are: a comment in `dev.daemon.yaml:118`; a **test fixture** in
`packages/tddy-supervisor/src/policy.rs:487` using `("SSH_AUTH_SOCK", "/tmp/agent")` as the example of
a **denied** env key; and the whole-work discovery doc.

⚠ **That test fixture is a warning, not a coincidence.** `resolve_env`
(`packages/tddy-supervisor/src/policy.rs:124`) is an **allowlist**: any key not listed is
`SupervisorError::Denied`. Under a supervisor the daemon is an unprivileged declared child
(CLAUDE.md), so **`SSH_AUTH_SOCK` may not be in the daemon's own environment at all**. Whether the
daemon can see the agent socket in a supervised deployment must be established in the red phase; it is
the single largest risk in this node, and it also decides whether node 6 is reachable in production.

### The decision taken: speak the agent protocol, do not shell out

Recorded in the whole-work discovery under *Design decisions*: **add an ssh-agent protocol crate**,
with explicit developer consent for the new dependencies (CLAUDE.md § ASK).

Why it matters even for this read-only node: listing keys via `ssh-add -l` means parsing
human-readable output whose format is not an API, and distinguishing "no agent" from "agent with no
keys" from "ssh-add missing" from exit codes. The wire protocol returns a typed list, and the
`REQUEST_IDENTITIES` / `IDENTITIES_ANSWER` exchange answers all four cases unambiguously.

The dependency is added **here**, in the read-only node, so node 6 inherits a proven connection rather
than introducing the crate and the passphrase flow at once.

### What a key listing can and cannot include

`IDENTITIES_ANSWER` returns, per identity, the **public key blob** and a **comment**. From those the
daemon can derive: key type, a fingerprint (SHA256 of the blob, as `ssh-add -l` displays), and the
comment — commonly the key's origin path or `user@host`.

⚠ **It cannot report which file a key came from.** The agent does not know. A row must not present the
comment as a path, because a comment is arbitrary text set at key-generation time.

### Which user's agent

Both `git config` (node 4) and an ssh-agent are **per-user**. `SSH_AUTH_SOCK` is a per-session
environment variable, so "the host's ssh-agent" is only meaningful relative to an OS user, and the
socket path is not discoverable from another user's environment.

`spawner::run_capture_as_user` (`packages/tddy-daemon/src/spawner.rs:736-830`) sets `HOME` and `PATH`
but **`PATH = merge_spawn_child_path(None)`** — it constructs the child environment rather than
inheriting a login session's. So it does not automatically carry an `SSH_AUTH_SOCK`.

→ **How the daemon locates the agent socket for the target user is this node's central design
  question**, and it must be settled in the PRD rather than discovered during `/green`. It is also
  what makes this node genuinely more than "one more probe".

### The RPC shape is node 4's

Node 4 owns the host tooling probe RPC and its response message. This node **adds a block** to that
response rather than creating a second near-identical RPC. That is recorded in this node's
`## Dependencies` as a surface it consumes and extends, not one it may redesign.

### The existing hardening this node must not fight

`packages/tddy-core/src/worktree.rs:39-52` — `git_remote_command` sets `GIT_TERMINAL_PROMPT=0` and
`.stdin(Stdio::null())` so a missing key or passphrase "fails fast instead of blocking forever — a
headless daemon has no TTY to answer such a prompt". `packages/tddy-daemon/src/config.rs:192-200`
documents the workaround: an `ssh_command` using `BatchMode=yes` with the macOS Keychain.

This node **reads agent state and changes nothing**, so none of that hardening is touched. Node 6 is
where it has to be revisited.
