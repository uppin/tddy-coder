# Initial discovery — `#carve` 2/9 `recipe-parsers`
...
## Exploration 4 — node-specific: the seams in `parser.rs` and the two hooks files

Run while writing this node's PRD, to establish that the six parser phases are genuinely independent
and to find the operation order the extraction needs.

### Sequence

1. `grep -n '^pub fn parse_\|^pub struct \|^struct \|^pub enum \|^impl ' src/parser.rs` — the full
   item list with line numbers, to find the phase boundaries.
2. The same restricted to `800 < line < 1000`, because the first pass showed the Evaluate DTOs and
   `impl RedOutput` interleaving there.
3. `grep -n '^pub fn \|^fn \|^impl \|^pub struct \|^const ' src/tdd/hooks.rs` — the hooks shape.
4. The same for `src/tdd_small/hooks.rs`.
5. `grep -rhoE 'tddy_workflow_recipes::parser[a-z_:]*'` across the workspace — external reach.

### Grep — notable hits

**`parser.rs` phase boundaries** (item start lines):

| Phase | Items | Range |
|---|---|---|
| planning | `PlanningOutput` 12, `DemoMode` 35, `PortMap` 42, `DemoPlan` 49, `DemoStep` 73, `StructuredPlan` 80, `parse_planning_response` 92, `…_with_base` 97, `prd_value_looks_like_md_file_path` 105, `…_impl` 114 | 12–154 |
| acceptance_tests | `AcceptanceTestsOutput` 155, `AcceptanceTestInfo` 176, `impl` 183, `StructuredAcceptanceTests` 223, `AcceptanceTestInfoDe` 241, `parse_acceptance_tests_response` 249 | 155–289 |
| analyze | `AnalyzeOutput` 290, `StructuredAnalyze` 300, `parse_analyze_response` 310 | 290–338 |
| green | `GreenOutput` 339, `DemoResults` 352, `DemoOutput` 359, `GreenTestResult` 371, `ImplementationInfo` 381, `StructuredGreen` 389, three `…De` 402/408/418, `parse_green_response` 426, `impl GreenOutput` 477 | 339–552 |
| red | `RedOutput` 553, `MarkerInfo` 582, `MarkerResult` 594, `RedTestInfo` 604, `SkeletonInfo` 613, `StructuredRed` 621, four `…De` 646/657/666/674, `parse_red_response` 682, `validate_red_marker_source_paths` 774 | 553–807 |
| evaluate | `EvaluateBuildResult` 808 … `EvaluateTestImpactDe` 882, **then** `EvaluateChangedFile` 975, `EvaluateAffectedTest` 984, `EvaluateOutput` 992, `StructuredEvaluate` 1006, two `…De` 1029/1039, `parse_evaluate_response` 1047 | **808–882 ∪ 975–1216** |

**`impl RedOutput` is at 887–974** — inside the Evaluate range, splitting it in two.

**`tdd/hooks.rs`** — `TddWorkflowHooks` 46, `impl TddWorkflowHooks` 52, then eleven `before_*` free
functions (146 `before_interview`, 207 `after_interview`, 229, 238, 296, 352, 386, 418, 449, 480),
nine `after_*` free functions (533 `after_plan`, 597, 628, 660, 676, 686, 711, 720, 729), then
`impl RunnerHooks for TddWorkflowHooks` at 737.

**`tdd_small/hooks.rs`** — same shape inverted: free functions first (42 `before_merged_red`, 91,
118, 149, 202 `after_plan`, 266, 298, 311, 361, 370), then `TddSmallWorkflowHooks` 380,
`impl TddSmallWorkflowHooks` 386, `impl RunnerHooks` 468.

### Findings

**The six parser phases are independent.** Each owns its output struct, a private `Structured…`
mirror, zero or more `…De` deserialization mirrors, and one `parse_*_response`. No phase names
another. The only symbol crossing every seam is `ParseError`, which stays in the parent.

**The Evaluate seam is non-contiguous and that dictates the operation order.** `extract_module`'s
anchor is a range over a selection of items, so a seam broken in two by an unrelated `impl` needs
either two operations or a prior extraction that closes the gap. Extracting **red before evaluate**
closes it: `impl RedOutput` leaves with the Red seam and the Evaluate items become one range.

**The hooks files carry no refusal risk.** The plan-schema records one blocking geometry — *"one
member is lifted out of an `impl` while a sibling in that same `impl` calls it"* — and this is not
it. The phase functions are **free functions**, not `impl` members; `impl RunnerHooks` calls them
from outside, which is the "path-reached item moves; the parent still names it" geometry the schema
records as **succeeding**, because the assist rewrites the reference and the import pass restores the
binding.

**Nothing in this node is cross-crate**, so `move_module_to_crate` — and therefore `#carve` 1/9's
fix — is not on its path. That is what puts it in wave 1.
