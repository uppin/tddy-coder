...
## Exploration 4 — node-specific: the storage group's one edge out, and what `sqlx` really costs

Run while writing this node's PRD. It produced **one dependency-edge correction to the stack plan**.

### Sequence

1. Production-only `crate::` census of `atomic_file.rs`, `error.rs`, `output/`, `session_actions/`,
   `session_catalog/` — comments stripped, truncated at the first `#[cfg(test)]`.
2. `grep -rln jsonschema .` and `grep -rln sqlx .` inside `tddy-core/src`.
3. `grep -rl "crate::<module>" --include='*.rs' . | wc -l` per moving module — reach inside the crate.
4. `cat error.rs` in full, once the census showed an unexpected `backend` edge.

### Grep — notable hits

**Outbound production dependencies of the moving group:**

```
atomic_file.rs   ->  (nothing)
error.rs         ->  backend
output/          ->  atomic_file  error
session_actions/ ->  atomic_file  output  session_actions
session_catalog/ ->  session_actions
```

**`sqlx` users — four files, all in one directory:** `session_catalog/{error,populate,store,read}.rs`.

**`jsonschema` users — two files, and only one of them moves:** `session_actions/validate.rs`
(moves) and `session_action_pipeline.rs` (**stays**).

**Reach inside `tddy-core`:** `crate::error` 14 files, `crate::atomic_file` 11,
`crate::session_actions` 9, `crate::output` 4.

### `error.rs` in full — the single edge out

62 lines, three enums (`BackendError`, `WorkflowError`, `ParseError`), and exactly one import from
...
consumed by one variant:

```rust
#[error("clarification needed: {questions:?}")]
ClarificationNeeded { questions: Vec<ClarificationQuestion>, session_id: String },
```
...
`n3`'s two) is recorded in the stack report rather than acted on, since renumbering a pushed stack
costs more than the ordering gains when both nodes are in the same wave.

**`jsonschema` does not leave, and the whole-work note implying it did is wrong.**
`session_action_pipeline.rs` stays in `tddy-core` and still names it. Only `sqlx` leaves.

**`sqlx` is the real prize.** Four files, 854 lines, one bundled C SQLite, compiled by **35 crates**.
The manifest alone will not prove the removal — a path dependency can re-introduce it — which is why
AC2 is a `cargo tree -p tddy-coder | grep sqlx` that must come back empty.
