# Initial discovery — optional-livekit node `host-directory`
...
## Exploration 2 — node-specific: the directory's current implementation

### The join path, end to end

`SelectedDaemonProvider` (`src/rpc/selectedDaemon.tsx`)
-> `useCommonRoomDaemons(livekitUrl, commonRoom, roomOverride, daemonsOverride, roomFactory)`
-> `useCommonRoom(url, roomName, identity, roomFactory)` (`src/hooks/useCommonRoom.ts`, 212 lines):

1. Guard — `if (!u || !rn || !id) { status "idle"; room null; return }`. **The unconfigured case
   already short-circuits before any LiveKit object is built.** That is the behaviour node 2 must
   preserve while still yielding the serving host.
2. `tokenClient.generateToken({room, identity})` via `useHttpClient(TokenService)`, plus a refresh
   timer at `ttl - 60s`.
3. `roomFactoryRef.current()` -> `new Room()`, event logging, `await liveRoom.connect(u, res.token)`.
4. `useObservedCommonRoomStatus(room)` maps `ConnectionState` -> `CommonRoomStatus` and reports a
   drop after a successful join.

Published status rule (worth preserving verbatim): *until there is a room object it is the outcome
of the join attempt; once there is one it is that room's own live connection state.*

### The identity and advertisement rules

- Presence identity: `presenceIdentityForUser(user.login)` (`src/lib/presenceIdentity.ts`), gated on
  `isAuthenticated`.
- Host derivation: `daemonHostsFromParticipants` over `useRoomParticipants(room)`; each host comes
  from `parseDaemonAdvertisement(metadata)` — a JSON object with `instance_id` and a `label`
  containing the literal `"(this daemon)"`, optionally `repos_base_path` and `max_attachment_bytes`
  (a cap of 0 or less reads as unadvertised). `inferParticipantRole` classifies `web-*`/`browser-*`
  as browser, `server*`/`daemon-*` as coder, advertisement-bearing as daemon.
- The daemon mirrors this in `livekit_peer_discovery.rs` (`eligible_daemon_from_participant_fields`,
  `daemon_rpc_identity`).

### Selection rules that must not change

`useSelectedDaemonState` (`selectedDaemon.tsx`):
- initial value from `resolveSelectedDaemonInstanceId({daemons, servingInstanceId, storedInstanceId,
  urlInstanceId})` (`src/routing/selectedHost.ts`, 51 lines — pure and already unit-testable);
- an **empty `daemons` list never clears a selection** — it reads as "no information yet", because
  the common room is briefly empty on connect and on reconnect;
- the resolved host is written to the `host` URL param with `replace: true`;
- an explicit `selectDaemon` writes `sessionStorage`, pushes a history entry, and drops the
  sub-selection;
- `<Fragment key={selectedInstanceId}>` remounts the whole screen subtree on host change while the
  provider itself stays mounted above the key.

The empty-list rule interacts with this node: once the serving host is always present, `daemons` is
never empty in the configured case either, so the rule stops being load-bearing for the common case
but must stay for the transient-disconnect case.

### The three presence consumers

| File | Use |
|---|---|
| `src/components/livekit/LiveKitAppPage.tsx:21-22` | `const { room, roomStatus, roomError } = useSelectedDaemon()` then `useRoomParticipants(room)` — the whole page is presence |
| `src/rpc-playground/RpcPlaygroundAppPage.tsx:81-82` | `useRoomParticipants(room)` to list RPC targets |
| `src/components/sessions/SessionsDrawerScreen.tsx:315-317,856` | `useRoomParticipants(room)` to reconcile cross-host sessions observed as common-room coder participants; `room={room}` passed down at :856 |

The third is the interesting one: cross-host session observation is a **presence** feature, so on a
presence-less connection the drawer must fall back to whatever `ListSessions` reports. That fallback
is node 4's gating decision; node 2 only has to make the dependency explicit.

### Where the serving host id comes from

`ClientConfig` in `packages/tddy-daemon/src/server.rs` carries `daemon_instance_id`, served at
`/api/config`, and `SelectedDaemonProviderProps.servingInstanceId` already receives it. No daemon
change is needed to contribute a serving-host descriptor.
