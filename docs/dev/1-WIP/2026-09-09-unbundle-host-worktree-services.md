...
**Stack**: `#unbundle` node **1 of 8** — the root. Base: `feature/connection-service-split/lsp-settle-budget` (PR #468)
...
It also introduces the **shared `types.proto`** that every later node imports — the most consequential
decision this node makes for the rest of the stack.
...
- `packages/tddy-service/proto/types.proto` with the shared messages, and `host.proto` / `worktree.proto`
  declaring all 17 methods — so nodes 6–8 can see the shared-types pattern before writing their own.
...
- [ ] **Proto**: `types.proto`, `host.proto`, `worktree.proto`; `build.rs` passes; sandbox extern paths re-pointed
