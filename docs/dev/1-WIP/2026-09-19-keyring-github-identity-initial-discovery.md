# Initial discovery — `#keyring` 9/9 `github-identity`

**Node**: `#keyring` 9/9 — github-identity · branch `feature/keyring/github-identity` · base `feature/keyring/link-github (#515)`
**Changeset**: [2026-09-19-keyring-github-identity.md](./2026-09-19-keyring-github-identity.md)
...
## Exploration 2 — the two places an identity is chosen today, and the PR standing in the way

**Scope**: `#keyring` 9/9 only. Read: `packages/tddy-workflow-recipes/src/github_rest_common.rs`,
`packages/tddy-daemon-livekit/src/session_room.rs:325-370`,
`packages/tddy-daemon-auth/src/github_token_store.rs:1-30`,
`packages/tddy-workflow-recipes/docs/code-issues/squatting-github-rest-client.md`, and `gh pr view 492`.

### 2.1 The token: one per process

```rust
/// Resolve GitHub token from the environment (`GITHUB_TOKEN` preferred, then `GH_TOKEN`).
```
— `github_rest_common.rs:17`

Every REST helper in the file funnels through it, and each fails with its own message when nothing is
set (`curl_github_post_json: no GitHub token set (GITHUB_TOKEN / GH_TOKEN)`). The consequence is
structural rather than a bug: **a daemon has one GitHub identity**, and every project it serves acts
as that identity.

`tddy-tools/src/server.rs:1480` builds on the same fact — the PR tools appear only *"When
**GITHUB_TOKEN** or **GH_TOKEN** is set"*. After this node that gate becomes per-session and per-
project for free, with no change in `tddy-tools`: a session whose project has an account has the
variable set; one without has the tools correctly absent.

### 2.2 The second, unrelated store

`github_token_store.rs`' doc comment is worth reading in full because it is **right about its own
problem and silent about this one**:
...
So there are two mechanisms — a process environment variable and a `login -> access_token` JSON file
— and **neither knows what a project is**. 3/9 replaced the storage; this node removes the last
readers of the file, and deletes the environment path.

### 2.3 The identity: nowhere, with one deliberate exception

For the agent's own commits, nothing sets an identity — the checkout's `user.name` / `user.email`
decide, which on a daemon host is whatever the machine carries.

The exception is `session_room.rs:337`, and it is **not** something this node should "fix":

> Signed by the daemon under a fixed identity rather than by whatever `user.email` the checkout
> happens to carry: this object is not the agent's work, it is a machine-made snapshot of it, and
> `git commit-tree` refuses outright in a repository that has configured no identity at all.

`WIP_COMMIT_IDENTITY_NAME = "tddy-daemon"`, `..._EMAIL = "tddy-daemon@tddy.invalid"`. A WIP snapshot
does not become the person's work when their project gains an account. Attributing it to them would
be *worse* after this node than before, because the attribution would then look deliberate.

**Worth stating as a finding**: the one place in the tree that already thought carefully about commit
identity concluded the machine should own it, for objects the machine makes. This node owns the other
case — objects the person's agent makes — and leaves that one alone.

### 2.4 Why the token and the identity must come from **one** resolution

They are two reads of the same answer, and the temptation is to resolve where each is needed. That
works until an assignment changes between the two calls, and then produces a commit **authored by
account A and pushed by account B** — which reads to everyone downstream as A's work, done by
someone who is not A.

The test is therefore written as a construction property (the environment is built from a single
record) rather than as a comparison of two lookups, because comparing two lookups is exactly the
implementation being ruled out.

### 2.5 The blocker, measured

`gh pr view 492`: **OPEN, draft**, `#carve` 6/10 `git-plumbing`, `feature/carve/git-plumbing` →
`feature/carve/core-foundations` — itself mid-stack, so its own landing waits on #488, #489, #490,
#498, #491 per the code-issue record.

The record, `squatting-github-rest-client.md`:

| | |
|---|---|
| Location | `github_pr.rs`, `github_rest_common.rs`, `orchestrate_pr_stack/github.rs` |
| Metrics | **2,124 lines**, consumed across a crate boundary by `tddy-tools` |
| Status | `Open — claimed by #492, in flight` |
| Guidance | *"Coordinate if you are adding a REST call — it belongs in `tddy-github` after #492"* |

This node does more than add a call: it changes **how all 2,124 lines get their token**. Before #492,
that means editing a crate the files are leaving and then having the move rewrite the result.

**The fork this would normally trigger was already settled by the developer**, in the instruction that
opened the stack:
...
Hence ⛔ BLOCKING, recorded, not escalated. **Nodes 1–8 touch none of those files**, so the block costs
this node's schedule and nothing else.

### 2.6 The seam, and the alternative that was not taken

Setting `GITHUB_TOKEN` and the four `GIT_*` variables in the session environment makes `git`, `gh`
and the agent's own calls correct with no tool learning anything about this stack.

It also puts the token where the agent can print it. A credential helper never exposes it to the
session — and does not cover `gh` or the agent's API calls, which *need* it, and is a much larger
change. Recorded as a decision with its alternative rather than settled silently; a helper is
additive later and does not disturb the resolution this node builds.

### 2.7 What "no fallback" means here, concretely

CLAUDE.md forbids fallbacks without explicit consent, and the developer's stack instruction repeats
it. This is the node where the rule has teeth: a fallback to the daemon's `GITHUB_TOKEN` when a
project has no account assigned turns *"this project has no GitHub account"* into *"pushed
successfully"* — **as whoever exported that variable**, to a repository that person can write to.

The single acceptance test that catches it is `NotAssigned` **with** `GITHUB_TOKEN` exported. Every
other test in the plan passes with the fallback present.
