...
**Wave:** **5 of 5 — alone.** Moved from 4 by the local-socket policy, which put nodes 7 and 8 into
a wave of their own behind node 6.
...
Real dependency edges, as opposed to the branch line:
    ...
    n1 → n2, n3, n4, n5      n2 → n4      n5 → n6, n7, n8      n6 → n7, n8
    n1, n3, n4, n6, n7, n8 → n9

    w1  n1
    w2  n2, n3, n5          3 concurrent
    w3  n4, n6
    w4  n7, n8              2 concurrent
    w5  n9                  alone

The local-socket policy deepened the graph from four waves to five: nodes 7 and 8 now consume node
6's `generate_tonic_adapter` — its *behaviour*, not its published surface — so they left wave 3.
...
- [ ] **`local_socket_server.rs`** becomes a multi-service tonic server carrying every service the
      stack moved onto the socket — nodes 1, 7, 8 and this one, plus family T's gap if node 4's is
      closed by then
- [ ] **Adapters generated**: this node's two are produced by node 6's `generate_tonic_adapter`, not hand-written
...
- **The local-socket policy applies here too, and this node is the only one where it is not
  hypothetical.** Nodes 7 and 8 keep their families reachable because the policy says so; this node
  keeps `session.SessionService` and `local_token.LocalTokenService` reachable because
  `tddy-sandbox-app/src/daemon_client.rs` **actually dials both** — `start_session` and
  `mint_local_token` are the only two methods a production tonic client calls over that socket. Its
  two adapters are generated by node 6's generator, and `local_socket_server.rs` becoming
  multi-service is what carries every other node's services alongside them.
