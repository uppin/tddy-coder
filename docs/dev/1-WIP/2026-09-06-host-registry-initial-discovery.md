# Initial discovery — `host-registry` (#hosts-screen 1/8)
...
## Exploration 5 — node-specific: what this node must build on

### The discovery path this registry hooks into

`CommonRoomPeerRegistry` — `packages/tddy-daemon/src/livekit_peer_discovery.rs:267-269`:

```rust
RwLock<HashMap<String, EligibleDaemonInfo>>
```

replaced **wholesale** by `sync_from_room` (`:277`) on a 500 ms tick plus participant events, with a
2 s / 6 s reconnect backoff on `DuplicateIdentity`. This is the single point where the daemon learns
a peer exists — and the single point where it forgets one. **The registry write goes here**: record
on appearance, stamp `last_seen` on disappearance, never delete.

`DaemonAdvertisement` (`:117-131`) is the payload already carried in participant metadata and is the
source of `label`, `repos_base_path`, `max_attachment_bytes`. `merge_discovered_peers_ordered`
(`:171`) puts the local entry first and dedups by `instance_id`.

The local entry comes from `packages/tddy-daemon/src/multi_host.rs:32`
`local_daemon_instance_id() -> DaemonInstanceId` (short hostname, else `"local"`), and
`:63 local_eligible_daemon_entry()`.

### The live source to intersect against for `online`

`trait EligibleDaemonSource` — `packages/tddy-daemon/src/multi_host.rs:17-29`, with
`fn list_eligible_daemons(&self) -> Vec<EligibleDaemonInfo>`. Wired in
`packages/tddy-daemon/src/runtime.rs:535-541` as `LiveKitEligibleDaemonSource`. A stub for tests
exists: `StubEligibleDaemonSource` (`multi_host.rs:75-78`).

→ `online` is computed per call as `registry ∩ list_eligible_daemons()`. Never stored.

### Precedent for the persistence shape

`packages/tddy-daemon/src/github_token_store.rs:38-73` — `FileGitHubTokenStore`, with
`::new(auth_storage_dir)`, `::tokens_path()`, `::probe_writable()`, owner-only file permissions, and
the rule that **a failed write fails the operation** rather than being swallowed. The same posture
applies here: a corrupt/absent file reads as empty, but a failed *write* must surface.

### The screen template, concretely

`VmsAppPage` / `VmsScreen` is the closest shape (a plain scrolling table screen, no drawer).
Touch points confirmed:

1. `packages/tddy-web/src/routing/appRoutes.ts` — `VMS_ROUTE` (`:89`) / `isVmsPath` (`:91`) is the
   pattern to copy for `HOSTS_ROUTE` / `isHostsPath`.
2. `packages/tddy-web/src/index.tsx:480-500` — the ternary dispatch chain; screen imports `:56-64`,
   predicate imports `:65-76`.
3. `packages/tddy-web/src/components/shell/DaemonNavMenu.tsx` — entries at `:74-156`, each a
   `Button role="menuitem"` with a `data-testid` of the form `shell-menu-<screen>`.
4. `packages/tddy-web/src/components/shell/AppShell.tsx:32` — `variant="scroll"` for a content screen.

### Testing surface confirmed for this node

- Rust: in-crate `#[tokio::test]` against the service struct, injecting `StubEligibleDaemonSource`
  and a temp-dir-backed registry. Pattern: `connection_service.rs:18019-18118`.
- Web: `mountWithRpc(withSelectedDaemon(<HostsAppPage />), backend)` with
  `backend.implement(ConnectionService, { listKnownHosts: async () => ... })`.
  `anInMemoryRpcBackend` records **unary** calls, so `callsTo(ListKnownHosts)` works directly here
  (the streaming-counter workaround is only needed from node 2 onward).
