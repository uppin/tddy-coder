# Initial discovery — `desktop-probe` (#hosts-screen 7/8)

**Node:** report a host's VNC and RDP reachability
...
## Exploration 5 — node-specific: what "available" can honestly mean

### Two different availabilities, and the row must not conflate them

**(a) Can this host's daemon *bridge* a desktop at all** — i.e. is the bridge binary present?

`packages/tddy-daemon/src/config.rs:451-500` — `vnc_binary_path`, `rdp_binary_path`,
`resolve_vnc_binary_path(config)`, `resolve_rdp_binary_path(config)`. Resolution order: explicit
config → sibling of `current_exe()` → bare name on `PATH`.

⚠ **This is a path *guess*, not an existence check.** There is no `Path::exists()`, no probe. A missing
binary surfaces only as a spawn `error!` at `screen_sharing_service.rs:174` — i.e. after the user has
already asked for a stream. Adding the `exists()` check is squarely this node's job and is the cheapest
real improvement in it.

**(b) Is there a desktop *to* connect to** — is something listening on the VNC/RDP port?

Nothing anywhere does this. No `TcpStream::connect` reachability probe exists in the repo for this
purpose.

→ The row must distinguish these. "tddy cannot bridge on this host" and "nothing is serving a desktop
  on this host" are different problems with different fixes, and collapsing them would send an operator
  to the wrong place.

### The existing control plane is per-session, not per-host

`packages/tddy-service/proto/screen_sharing.proto:8-15`:

```proto
service ScreenSharingService { ListTargets, AddTarget, RemoveTarget, UnlockVault, StartStream, StopStream }
enum Protocol { PROTOCOL_UNSPECIFIED, PROTOCOL_VNC = 1, PROTOCOL_RDP = 2 }
message ScreenSharingTarget { id, label, host, port, protocol, username }
```

**Every request carries `session_token` + `session_id`** (e.g. `ListTargetsRequest:38-41`), and the
credential vault lives under the **session directory** (`screen_sharing_vault.rs`). Legacy
`vnc.proto:7-13` (`VncService`) is still present alongside it.

→ A **host-scoped** target model does not exist. Node 8 owns creating it; this node only *reports*, so
  it does not need one — which is exactly why probe and connect are separate nodes.

### Ports

Default VNC is 5900 + display number; default RDP is 3389. A host may serve on either, both, or a
non-default port. A probe that only checks the defaults must **say** that is what it checked, rather
than reporting a bare "unavailable" that an operator reads as authoritative.

### `Protocol` already exists — reuse it

`screen_sharing.proto:19-23` already defines `Protocol { UNSPECIFIED, VNC, RDP }`. This node should
reuse that enum rather than defining a parallel one on `ConnectionService`, even though the probe lives
on a different service. Two enums meaning the same thing is how they drift.

### The RPC shape is node 4's, again

Like node 5, this node **adds a block** to node 4's host tooling probe response rather than creating a
third near-identical RPC. Recorded in `## Dependencies` as consumed-and-extended, not owned.

### Bounded, and cheap

A reachability probe must be **bounded by a short timeout** — an unreachable host must not hold the
tooling RPC open — and must be **non-intrusive**: a TCP connect that closes immediately, never a
protocol handshake. A half-open RFB handshake against someone's desktop is a rude thing for a
monitoring screen to do on a timer.
