...
- ~~`--specialized-agent` CLI flag + deprecated aliases~~ — done (2026-07-02, multi-agent tool-replacement changeset). `tddy-sandbox-app` now takes repeatable `--specialized-agent <name>` + `--agents-dir`, resolves them (plus any single-agent `--fastcontext-*`/`--subagent-replaces` overrides) via `spawn::resolve_specialized_agent_names`/`resolve_specialized_agents`, and threads the resolved array into the jail as `TDDY_SUBAGENT`/`TDDY_SUBAGENTS_JSON` via the rewritten `spawn::subagent_env_overlay`. `--discovery-subagent` remains a deprecated single-name alias, mutually exclusive with `--specialized-agent` (hard error, not silent precedence). See `docs/ft/coder/managed-codebase-subagents.md` ACs 19-24 and `docs/ft/coder/specialized-subagents.md` ACs 11-12.
