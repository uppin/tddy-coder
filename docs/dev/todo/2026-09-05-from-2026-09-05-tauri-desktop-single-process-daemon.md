...
- Writing the daemon config **loses the operator's YAML comments** (`dev.desktop.yaml` is heavily commented). Field values round-trip safely — verified against `dev.daemon.yaml`, `dev.desktop.yaml` and `config.example.yaml`. Preserving comments needs a comment-aware YAML editor, i.e. a new dependency. `TODO` at `packages/tddy-daemon/src/daemon_config_service.rs`. **A second writer has the same loss**: `enrol_first_login` in `packages/tddy-daemon-kernel/src/first_login_enrolment.rs`, added by `#keyring` 2/9 ([#509](https://github.com/uppin/tddy-coder/pull/509)), edits the file as a `serde_yaml::Value` so only `users:` changes, but serialising it back strips every comment. It runs on **every desktop's first GitHub sign-in**, so the explanatory header `./install --desktop` renders from `desktop.yaml.production` is gone from `~/.tddy/desktop.yaml` after the first login. `TODO` at the write in `enrol_first_login`. One open question (a comment-aware YAML editor), two writers, one fix: both should go through the same editor once it exists.
