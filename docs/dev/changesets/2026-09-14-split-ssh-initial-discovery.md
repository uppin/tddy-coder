# Initial Discovery: split sessions pick SSH execution on the codebase host (`#ssh-exec` 4/4)

**Changeset**: [2026-09-14-split-ssh.md](./2026-09-14-split-ssh.md)
**Date**: 2026-09-14
**Passes**: 3

## Combined Conclusions

**This node (n4)** owns the create-session SSH dropdown when the session is split: aliases come
from **codebase host B** (`ListSshConfigHosts` with `daemon_instance_id` = B), and
`ssh_config_host` is forwarded on the workspace `StartSession` so B's n2 RemoteShell runs exec
tools on T. Agent on A still reaches B over LiveKit `ExecuteTool`.

State A: `CreateSessionPane` already has `create-session-codebase-host-select` and
`isSplitCodebase`. n2 adds `create-session-ssh-config-select` sourced from Host A. Split start
(`start_split_claude_cli_session` → `workspace_start_request` with `..req.clone()`) would copy
an `ssh_config_host` field if present — but listing A's aliases for a B-side RemoteShell is
wrong (B's `~/.ssh/config` is the client config). n4 retargets the list and pins the forward
so A does not apply SSH locally (A has no repo).

n1 list RPC, n2 RemoteShell, n3 Serve-over-SSH are consumed, not reimplemented.
...
## Exploration 3: n4 split start + dropdown seam — 2026-09-14

**Agent**: parent Read
**Scope**: `CreateSessionPane` codebase host select, `workspace_start_request`, `start_split_claude_cli_session`

### Findings

`canChooseCodebaseHost` is claude-cli + managed + daemon list. `isSplitCodebase` is a different
daemon than the agent host. `HostAddKeyAction` already addresses `ListHostKeyCandidates` with
`daemonInstanceId: instanceId` — n4's dropdown must do the same for `ListSshConfigHosts` using
the **codebase** host id when split, the session host when co-located (n2 already does A).

`workspace_start_request` uses `..req.clone()`, so an `ssh_config_host` field on the operator's
StartSession would copy onto the workspace request unless A strips or consumes it first. n4
must: (1) list B's aliases in the UI, (2) keep the field on the forwarded workspace start,
(3) not materialize or SSH from A. LiveKit `ExecuteTool` already lands on B; B then uses n2's
RemoteShell. Do not absorb the split-room-on-restart sweep
(`2026-08-14-a-split-session-s-room-is-not-re-opened-when-the-daemon-restarts.md`).
