...
**Metrics:** **621 production lines** (2026-09-24, #510 after its idle-vault follow-up; 622 at its wrap; 576 on `origin/master` `35cf2913`), counted to the first `#[cfg(test)]` · budget 500 · ~1.24× over · 1,402 lines in all
...
| 2026-09-24 | 622 → 621 | #510's post-wrap follow-up: `−1`. `pending_logins.rs` became `vault_lifetimes.rs` when its sweep took on idle open vaults, and `credential_vaults_in` now takes the `github:` block, so the `let ttl` line went; the call is still rustfmt-wrapped. `github.open_vault_idle_ttl_seconds`, its startup logs and its sweep live in `vault_lifetimes.rs`, not here |
