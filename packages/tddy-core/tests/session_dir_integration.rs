...
async fn test_plan_goal_uses_session_dir() {
    ...
    // Provide session_base (new context key) but NOT plan_dir.
    // PlanTask should call create_session_dir_in(session_base) and set plan_dir to the UUID path.
    ...
    // Get plan_dir from session context after plan completes
    let session = engine
        .get_session(&result.session_id)
        .await
        .expect("get_session should work")
        .expect("session should exist");
    let plan_dir: std::path::PathBuf = session
        .context
        .get_sync("plan_dir")
        .expect("plan_dir should be set in session context after plan goal");

    // plan_dir must be under base/sessions/{uuid}/
    let sessions_dir = base.join("sessions");
    assert!(
        plan_dir.starts_with(&sessions_dir),
    ...
    let uuid_part = plan_dir.file_name().unwrap().to_str().unwrap();
    assert_eq!(
        uuid_part.len(),
        36,
    ...
    assert!(
        !plan_dir.to_string_lossy().contains(&today_prefix),
