...
- **planning**: System prompt (structured-response format) and user prompt construction. Writes system prompt to plan dir; stores initial_prompt and clarification_qa in changeset. Persists questions when ClarificationNeeded; pairs with answers on follow-up. After write_artifacts, relocate_plan_dir moves the plan dir to git_root/suggestion/dir_name when plan_dir_suggestion is present and valid (rejects absolute, .., empty; cross-device copy+delete fallback).
