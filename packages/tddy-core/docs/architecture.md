...
- **activity_prompt_log**: Stable prefixes **`User: `** and **`Queued: `**; **`format_user_prompt_line`** / **`format_queued_prompt_line`** supply lines for **`SubmitFeatureInput`** and **`QueuePrompt`**, logged with **`ActivityLogged`** via **`log_activity`**.
