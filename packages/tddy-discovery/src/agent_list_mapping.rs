//! Maps configured agent allowlist entries to display rows shared by ListAgents-style surfaces.

use tddy_service::proto::models::AssistantEntry;

/// One row in a `ListAgents`-style answer: backend id and the label shown in pickers.
...
/// A configured backend from the daemon operator's allowlist, before registry assistants are appended.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ConfiguredAgentAllowlistEntry {
    pub id: String,
    pub label: Option<String>,
}

/// The agents this daemon can start a session as: one row per configured allowlist entry,
...
pub fn agent_allowlist_rows(
    configured: &[ConfiguredAgentAllowlistEntry],
    ...
    let configured = configured.iter().map(|a| {
...
mod tests {
    ...
    fn agent_allowlist_rows_match_list_agents_label_rules() {
        ...
        let rows = agent_allowlist_rows(&configured, &[]);
    ...
    fn agent_allowlist_rows_blank_trimmed_label_falls_back_to_id() {
        let configured = vec![ConfiguredAgentAllowlistEntry {
            id: "only-id".into(),
            label: Some("   ".into()),
        }];
        let rows = agent_allowlist_rows(&configured, &[]);
