...
use tddy_workflow_recipes::parse_acceptance_tests_response;

use common::{ctx_acceptance_tests, run_plan, temp_dir_with_git_repo, write_changeset_for_session};
...
async fn acceptance_tests_workflow_reads_session_dir_and_invokes_backend_with_resumed_session() {
    ...
    .expect("write PRD");
    write_changeset_for_session(&session_dir, "sess-resume-123");
...
async fn acceptance_tests_workflow_passes_goal_allowlist_to_invoke_request() {
    ...
    assert_eq!(
        req.goal_id,
        tddy_core::GoalId::new("acceptance-tests"),
        "InvokeRequest should have goal_id acceptance-tests"
    );
...
async fn plan_workflow_passes_goal_to_invoke_request() {
    ...
    assert_eq!(
        req.goal_id,
        tddy_core::GoalId::new("plan"),
        "InvokeRequest should have goal_id plan"
    );
