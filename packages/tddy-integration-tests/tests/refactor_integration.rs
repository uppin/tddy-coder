...
async fn refactor_invokes_backend_with_refactor_goal() {
    ...
    write_changeset_with_state(&session_dir, "ValidateComplete", "sess-refactor-goal");
    ...
    let req = invocations
        .iter()
        .find(|r| r.goal_id == GoalId::new("refactor"))
        .expect("refactor invocation should exist");
    assert_eq!(
        req.goal_id,
        GoalId::new("refactor"),
        "InvokeRequest must have goal_id refactor"
    );
...
async fn refactor_transitions_to_refactor_complete() {
    ...
    std::fs::create_dir_all(&session_dir).expect("create plan dir");
    write_refactoring_plan(&session_dir);
    write_changeset_with_state(&session_dir, "ValidateComplete", "sess-validate-1");
...
async fn refactor_parses_structured_response() {
    ...
    std::fs::create_dir_all(&session_dir).expect("create plan dir");
    write_refactoring_plan(&session_dir);
    write_changeset_with_state(&session_dir, "ValidateComplete", "sess-refactor-parse");
    ...
    let ctx = ctx_refactor(session_dir.clone());
    let result = engine
        .run_goal(&GoalId::new("refactor"), ctx)
        .await
        .unwrap();
