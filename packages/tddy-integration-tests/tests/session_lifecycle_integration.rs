...
async fn changeset_exists_before_workflow_starts() {
    ...
    write_changeset(&session_dir, &init_cs).expect("write changeset before workflow");

    assert!(
        session_dir.join("changeset.yaml").exists(),
        "changeset.yaml must exist before workflow starts"
    );
    let cs = read_changeset(&session_dir).expect("read");
    assert_eq!(cs.state.current, WorkflowState::new("Init"));
...
async fn acceptance_tests_creates_fresh_session_no_crash() {
    ...
    .expect("PRD");
    write_changeset_with_state(&session_dir, "Planned", "plan-session-id-12345");
    ...
    let result = engine.run_goal(&GoalId::new("acceptance-tests"), ctx).await;
...
async fn green_resumes_from_state_session_id() {
    ...
    std::fs::write(session_dir.join("acceptance-tests.md"), "# Tests\n- login").expect("AT");
    std::fs::write(session_dir.join("progress.md"), "# Progress\n- red done").expect("progress");

    let mut cs = Changeset::default();
    cs.state.current = WorkflowState::new("RedTestsReady");
    ...
    write_changeset(&session_dir, &cs).expect("write");
