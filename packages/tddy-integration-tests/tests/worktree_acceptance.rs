...
- [ ] Implement
"#;
    fs::write(session_dir.join("PRD.md"), prd).unwrap();
...
fn test_context_header_includes_repo_dir() {
    ...
    let header = build_context_header(Some(&dir), Some(&repo_dir), &["PRD.md"]);

    assert!(
        !header.is_empty(),
        "header should not be empty when session_dir has artifacts"
    );
    assert!(
