...
use tddy_core::{GoalId, InvokeResponse, MockBackend, SharedBackend, WorkflowEngine};
...
fn setup_red_session_dir(session_dir: &std::path::Path) {
...
async fn red_workflow_passes_goal_allowlist_to_invoke_request() {
    ...
    assert_eq!(
        red_inv.goal_id,
        GoalId::new("red"),
        "InvokeRequest should have goal_id red for red workflow"
    );
