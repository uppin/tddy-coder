...
async fn tui_testkit_shows_feature_prompt() {
    // Given a stub presenter and a VirtualTui session
    ...
    // When waiting for the initial feature prompt
    // Then "feature description" appears on screen within the timeout
...
async fn tui_testkit_type_text_visible() {
    // Given a VirtualTui session at the feature prompt
    let _ = env_logger::builder().is_test(true).try_init();
    let (_handle, factory, shutdown) = spawn_stub_presenter(None);
    let session = start_virtual_tui_session(&*factory, false).expect("session");
    let tk = TuiTestkit::new(session, 80, 24);
    ...
    // When typing text into the TUI
    tk.type_text("hello").await.unwrap();

    // Then the typed text appears on screen
...
async fn tui_testkit_wait_for_text_times_out() {
    // Given a VirtualTui session
    let _ = env_logger::builder().is_test(true).try_init();
    let (_handle, factory, shutdown) = spawn_stub_presenter(None);
    let session = start_virtual_tui_session(&*factory, false).expect("session");
    let tk = TuiTestkit::new(session, 80, 24);

    // When waiting for text that will never appear
    let err = tk
        .wait_for_text("___NOT_ON_SCREEN_ZZZ___", Duration::from_millis(300))
        .await;

    // Then the wait returns an error (timeout)
...
async fn tui_testkit_resize_and_mouse() {
    // Given a VirtualTui session with mouse support enabled
    let _ = env_logger::builder().is_test(true).try_init();
    let (_handle, factory, shutdown) = spawn_stub_presenter(None);
    ...
    // When sending resize, click, and scroll events
    ...
    // Then no panics and the session remains active
    tk.shutdown();
    shutdown.store(true, Ordering::Relaxed);
}
...
async fn tui_testkit_screen_line_matches_screen_contents() {
    ...
    // When fetching the full screen and the first line separately
    let full = tk.screen_contents().await;
    let line0 = tk.screen_line(0).await;

    // Then the first line is a substring of the full screen content
    assert!(
...
async fn tui_testkit_press_enter_sends_input() {
    ...
    // Then a new render is triggered (no hang)
    tk.wait_for_render(Duration::from_secs(2)).await.unwrap();
    tk.shutdown();
    shutdown.store(true, Ordering::Relaxed);
}
...
async fn free_prompting_stub_responds_to_first_prompt() {
    // Given a VirtualTui session running the FreePromptingRecipe with StubBackend
    let _ = env_logger::builder().is_test(true).try_init();
    ...
    // Then the stub response appears on screen
...
async fn free_prompting_second_prompt_produces_new_response() {
    // Given a FreePromptingRecipe session that has received one prompt and responded
    let _ = env_logger::builder().is_test(true).try_init();
    let (_handle, factory, shutdown) =
        spawn_stub_presenter_with_recipe(None, Arc::new(FreePromptingRecipe));
    let session = start_virtual_tui_session(&*factory, false).expect("session");
    ...
    // When sending a second prompt after the first response
    ...
    // Then a second stub response appears, giving at least 2 total
...
async fn bugfix_demo_asks_two_questions_after_first_prompt() {
    // Given a VirtualTui session running the BugfixRecipe
    let _ = env_logger::builder().is_test(true).try_init();
    let (_handle, factory, shutdown) =
    ...
    // When the user submits a bug description
...
async fn tdd_full_workflow_interview_through_plan_approval() {
    // Given a VirtualTui session running the TDD recipe with StubBackend
    let _ = env_logger::builder().is_test(true).try_init();
    let (_handle, factory, shutdown) = spawn_stub_presenter(None);
    let session = start_virtual_tui_session(&*factory, false).expect("session");
    ...
    // When the user submits a feature and answers two interview questions
    ...
    // Then the plan approval menu appears with View/Approve/Refine options
    // (Interview handoff is merged into plan context — the first plan invoke skips clarification.)
