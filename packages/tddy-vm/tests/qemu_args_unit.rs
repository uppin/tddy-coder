...
/// A single port forward must format as `tcp::<host>-:<guest>`.
#[test]
fn qemu_args_hostfwd_formats_correctly() {
    ...
    let spec = QemuVmArgs::hostfwd_spec(&port_forward);
    assert_eq!(
        spec, "tcp::2222-:22",
...
// ── port-forward → share URL ─────────────────────────────────────────────────

/// The share URL for a PortForward must use `http://localhost:<host_port>`.
#[test]
fn port_forward_to_share_url_uses_host_port() {
    let port_forward = PortForward {
        host_port: 8080,
        guest_port: 80,
    };
    let url = format!("http://localhost:{}", port_forward.host_port);
    assert_eq!(
