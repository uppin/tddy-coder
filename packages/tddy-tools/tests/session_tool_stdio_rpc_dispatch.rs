...
//! `exec_tools.ExecToolService/ExecuteTool` with the same `ExecuteToolRequest`/
...
//! spawned child process hosting a fake `ExecToolService/ExecuteTool` handler that echoes
...
async fn dispatches_a_tool_call_over_stdio_rpc_and_returns_the_result_json() {
    // Given an RPC client wired to a fake ExecToolService/ExecuteTool handler over stdio
...
async fn round_trips_a_payload_larger_than_a_single_socket_read_without_truncation() {
    // Given an RPC client wired to a fake ExecToolService/ExecuteTool handler over stdio, and a
