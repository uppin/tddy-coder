...
- **2026-03-22** [Feature] Web-dev script contract — `web_dev_contract` module (`verify_*`, substring detectors) and `tests/web_dev_script.rs` for `bash -n` and daemon-only content checks; granular tests delegate to `verify_*`. `grpc_reconnect_acceptance::grpc_reconnect_second_stream_receives_full_tui_render` ignored pending reconnect/view-state fix. (tddy-e2e)
