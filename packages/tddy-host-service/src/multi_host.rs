...
///
/// The hostname lookup itself lives in [`tddy_daemon_kernel::daemon_identity`], because the config
/// fallback that uses it is read by the spawn layer as well as by this one, and two derivations of
/// "what is this machine called" is one too many.
pub fn local_daemon_instance_id() -> DaemonInstanceId {
    DaemonInstanceId(tddy_daemon_kernel::daemon_identity::local_daemon_instance_id_string())
}
