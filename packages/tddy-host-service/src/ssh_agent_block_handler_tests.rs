...
use tddy_service::proto::host::ProbeOutcome as WireProbeOutcome;
...
fn a_config_mapping_ada() -> tddy_daemon_kernel::config::DaemonConfig {
...
fn service_probing(tooling: HostTooling) -> (HostServiceImpl, Arc<FakeToolingProbe>) {
    ...
    let service = HostServiceImpl::new(a_config_mapping_ada(), temp.path(), user_resolver)
...
async fn host_tooling_reports_the_ssh_agent_block_for_the_hosts_os_user() {
    ...
    assert_eq!(agent.outcome, WireProbeOutcome::Ok as i32);
