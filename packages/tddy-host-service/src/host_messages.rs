//! Turning what a host probe found into what `host.HostService` puts on the wire.
//!
//! Every mapper here keeps "we could not find out" apart from "there is nothing" — an empty string
//! for a git identity, an empty key list for an agent and a `false` for a desktop each mean two
//! different things to the operator reading them, and collapsing the pair is how a probe failure
//! comes to read as a finding.

use tddy_service::proto::host::HostRemoteDesktop;
use tddy_service::proto::host::ProbeOutcome as ProtoProbeOutcome;

use tddy_service::proto::host::HostGithubCli;

use crate::ssh_agent_add::AgentAddFailure;

use tddy_service::proto::host::AddHostKeyOutcome;

use tddy_service::proto::host::AddHostKeyResponse;
...
use tddy_service::proto::host::SshAgentKey;

use tddy_service::proto::host::HostSshAgent;

use tddy_service::proto::host::HostGitIdentity;
