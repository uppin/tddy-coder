...
fn a_service_serving_two_operators(storage: &Path) -> HostServiceImpl {
    ...
    HostServiceImpl::new(config, storage, user_resolver)
}
...
async fn serves_an_add_addressed_to_this_daemon_by_its_own_instance_id() {
    ...
    let added = host
        .add_key_answering(
            AddHostKeyRequest {
                daemon_instance_id: host.service.local_instance_id(),
                ..host.an_add_of_their_key()
            },
            host.an_answer_carrying(PASSPHRASE),
        )
