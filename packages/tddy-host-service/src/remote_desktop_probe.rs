...
//! `resolve_rdp_binary_path` (`tddy_daemon_kernel::config`) resolve a path by *guessing* — explicit config, then
...
impl TcpRemoteDesktopProbe {
    ...
    /// [`tddy_daemon_kernel::config`] would spawn it from — an explicitly configured one included.
...
mod tests {
    ...
    fn a_config_with_vnc_binary_at(binary: &Path) -> DaemonConfig {
        DaemonConfig {
            screen_sharing: Some(tddy_daemon_kernel::config::ScreenSharingConfig {
