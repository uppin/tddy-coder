...
pub use tddy_daemon_kernel::spawn_as_user::{
    ...
    run_output_as_user, start_output_as_user, CaptureAsUser, SpawnedAsUser,
};
...
/// effective id as [`tddy_daemon_kernel::daemon_identity::local_instance_id_for_config`] so identities
...
pub fn livekit_spawn_daemon_instance_id(config: &DaemonConfig) -> Option<String> {
    if livekit_common_room_is_set(config) {
        Some(tddy_daemon_kernel::daemon_identity::local_instance_id_for_config(config))
    } else {
        config
            .daemon_instance_id
            .as_deref()
