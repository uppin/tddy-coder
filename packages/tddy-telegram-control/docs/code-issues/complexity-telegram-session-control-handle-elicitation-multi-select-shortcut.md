...
`packages/tddy-telegram-control/src/telegram_session_control/elicitation.rs` is 456 production lines, under the 500 budget, so no file-level record covers it. (The whole-file record for the unsplit `telegram_session_control.rs` was closed by #494's seven-module split.)
