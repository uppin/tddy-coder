//! The daemon's own notification subscriber: Telegram.
//!
//! PRD: `docs/ft/daemon/session-notifications.md` (FR2, FR3, FR7).
//!
//! The stream subscriber `tddy-web` reads lives in
//! [`tddy_session_activity::session_notification_subscribers`]. This one is Telegram's: it
//! delivers through [`TelegramDaemonHooks`] and reads the daemon's `telegram:` config block, so it
//! lives with the rest of the Telegram control plane and `runtime.rs` adds it to the daemon's bus.
...
use tddy_session_activity::session_notifications::{
