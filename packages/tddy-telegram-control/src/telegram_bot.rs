...
use crate::telegram_session_control::{
    ...
    parse_start_claude_prompt, parse_start_cursor_prompt, parse_start_workflow_prompt,
    parse_submit_feature_command, parse_telegram_agent_callback, parse_telegram_branch_callback,
    parse_telegram_branch_conflict_callback, parse_telegram_branch_more_callback,
    parse_telegram_chain_parent_callback, parse_telegram_claude_model_callback,
    parse_telegram_cursor_model_callback, parse_telegram_intent_callback,
    parse_telegram_project_callback, ChainWorkflowCommand, SessionControlCallback,
    StartClaudeCommand, StartCursorCommand, StartWorkflowCommand, TelegramCallback,
    TelegramSessionControlHarness, CB_TELEGRAM_CHAIN_PARENT,
};
...
async fn telegram_message_handler(bot: Bot, harness: Harness, msg: Message) -> HandlerResult {
    ...
    log::info!(
        target: "tddy_daemon::telegram_bot",
        "{}",
        tddy_telegram::telegram_tracked_session::format_inbound_message_traffic_log(
            chat_id,
            text.len(),
            None,
        )
    );
    tddy_telegram::telegram_tracked_session::log_inbound_telegram_message_body(chat_id, &text);
    ...
    if let Some(prompt) = parse_start_cursor_prompt(&text) {
        let cmd = StartCursorCommand {
            chat_id,
            user_id,
            prompt,
        };
        let mut h = harness.lock().await;
        if h.is_authorized(chat_id) {
            if let Err(e) = h.handle_start_cursor(cmd).await {
                bot.send_message(ChatId(chat_id), format!("{e:#}")).await?;
            }
        }
        return Ok(());
...
async fn telegram_callback_handler(bot: Bot, harness: Harness, q: CallbackQuery) -> HandlerResult {
    ...
    log::info!(
        target: "tddy_daemon::telegram_bot",
        "{}",
        tddy_telegram::telegram_tracked_session::format_inbound_callback_traffic_log(
            chat_id,
            data.len(),
            None,
            cb_prefix,
    ...
    if let Some((choice, proj_idx, session_id)) = parse_telegram_branch_conflict_callback(&data) {
        if workflow_callback_gate_authorized(&bot, &harness, chat_id, qid.clone()).await {
            return Ok(());
        }
        let _ = bot.answer_callback_query(qid.clone()).await;
    ...
    if let Some((model_idx, proj_idx, session_id)) = parse_telegram_cursor_model_callback(&data) {
        if workflow_callback_gate_authorized(&bot, &harness, chat_id, qid.clone()).await {
            return Ok(());
        }
        let _ = bot.answer_callback_query(qid.clone()).await;
        let h = harness.lock().await;
        match h
            .handle_telegram_cursor_model_callback(chat_id, model_idx, proj_idx, &session_id)
            .await
        {
            Ok(()) => {}
            Err(e) => {
