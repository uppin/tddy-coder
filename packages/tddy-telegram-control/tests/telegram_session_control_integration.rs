...
//! Integration tests for [`tddy_telegram_control::telegram_session_control`] (harness + sender recording).
...
async fn telegram_more_recipes_sends_second_keyboard() {
    ...
    assert_eq!(
        mr_buttons.len(),
        tddy_telegram_control::telegram_session_control::RECIPE_MORE_PAGE.len(),
