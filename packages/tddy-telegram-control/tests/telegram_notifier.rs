...
use tddy_telegram_control::telegram_notifier::{TelegramSender, TelegramSessionWatcher};
...
impl TelegramSender for MockTelegramSender {
    ...
    async fn send_message_with_keyboard(
        ...
        chat_id: i64,
        text: &str,
        _inline_keyboard: tddy_telegram_control::telegram_notifier::InlineKeyboardRows,
    ) -> anyhow::Result<()> {
        self.calls.lock().unwrap().push((chat_id, text.to_string()));
        Ok(())
    }
