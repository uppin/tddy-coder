...
fn phase2_live_tcp_dispatch_ready_is_false_until_green() {
    // When / Then
    tddy_telegram_control::telegram_bot::chain_phase2_tcp_dispatch_marker_probe();
    assert!(
        tddy_telegram_control::telegram_bot::session_chaining_phase2_live_tcp_dispatch_ready(),
...
fn phase2_chain_base_merge_ready_is_false_until_green() {
    // When / Then
    assert!(
        tddy_telegram_control::telegram_session_control::session_chaining_phase2_chain_base_merge_ready(),
...
fn merge_chain_integration_base_returns_ok_for_placeholder_paths_in_green() {
    ...
    // When / Then
    tddy_telegram_control::telegram_session_control::merge_chain_integration_base_with_explicit_operator_overrides(
        session_base,
        parent_id,
        &child_dir,
        &repo_canon,
