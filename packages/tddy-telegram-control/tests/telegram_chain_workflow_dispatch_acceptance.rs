//! Acceptance: inbound [`tddy_telegram_control::telegram_bot`] routes `/chain-workflow` like `/start-workflow`.

/// Telegram bot message dispatcher must parse `/chain-workflow` and delegate to
/// [`tddy_telegram_control::telegram_session_control::TelegramSessionControlHarness::handle_chain_workflow`].
#[test]
