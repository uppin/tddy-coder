...
use tddy_telegram_control::telegram_notifier::{
    InMemoryTelegramSender, InlineKeyboardRows, TelegramSessionWatcher,
};
use tddy_telegram_control::telegram_session_control::parse_elicitation_multi_select_shortcut;

const AUTHORIZED_CHAT: i64 = 424_242;

fn telegram_config() -> tddy_daemon_kernel::config::DaemonConfig {
...
fn telegram_choose_none_submits_empty_multi_via_presenter() {
    ...
    // When / Then
    let cb = tddy_telegram_control::telegram_multi_select_shortcuts::compose_choose_none_callback(
        sid, 0,
    );
    assert!(
