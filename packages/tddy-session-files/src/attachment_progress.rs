...
pub fn cleanup_materialized_attachments(session_dir: &Path, written: &[SessionAttachment]) {
...
pub fn attachment_size_bytes(session_dir: &Path, basename: &str) -> u64 {
...
pub struct AttachmentProgressSink {
    pub tx: Option<tokio::sync::mpsc::UnboundedSender<Result<StartSessionEvent, Status>>>,
}
...
impl AttachmentProgressSink {
    ...
    pub fn report(&self, progress: AttachmentMaterializationProgress) {
...
pub struct AttachmentProgressReporter<'a> {
    pub sink: &'a AttachmentProgressSink,
    pub basename: &'a str,
...
pub struct AttachmentMaterialization<'a> {
    pub session_token: &'a str,
    pub os_user: &'a str,
    pub sessions_base: &'a Path,
    pub session_id: &'a str,
    pub attachments: &'a [SessionAttachment],
    pub progress: &'a AttachmentProgressSink,
}
