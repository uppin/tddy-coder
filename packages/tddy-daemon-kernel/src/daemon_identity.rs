...
///
/// Defined in [`tddy_service::participant_identity`], beside the one rule that decides whether a
/// common-room participant may be a daemon — the rule both peer discovery and every client-facing
/// mint read, which is what makes the prefix a reservation rather than a convention. Re-exported
/// here, with the two daemon ids, for the crates that mint agents' identities; `split_session`
/// re-exports it in turn, so no caller's path changed and there stays exactly one definition.
pub use tddy_service::participant_identity::SPLIT_AGENT_IDENTITY_PREFIX;
