...
//! Module docs: `packages/tddy-session-agents/docs/session-agent-roster.md`.
//!
...
pub async fn run_clone_mirror(
    ...
    let identity = tddy_daemon_livekit::livekit_peer_discovery::daemon_rpc_identity(
        &spec.owning_daemon_instance_id,
    );
    let facilitator_identity = tddy_daemon_livekit::livekit_peer_discovery::daemon_rpc_identity(
    ...
    fn open(spec: &CloneMirrorSpec) -> Result<Self, Status> {
        ...
        let ssh_command = spec.facilitating_daemon_url.as_ref().map(|url| {
            format!(
                ...
                tddy_worktree_service::project_provision::resolve_remote_git_repo_path(),
                spec.session_token
            )
        });
        Ok(Self {
