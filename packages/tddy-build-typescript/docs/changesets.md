...
- **2026-07-23** [Feature] **bsp-build-server — mode-aware lowering** — `TypeScriptPlugin::lower_mode` emits configurable bun `test`/`start` scripts for `BuildMode::Test`/`Run` (Compile stays the build script); unsupported modes are rejected. Cross-package [docs/dev/changesets.md](../../../docs/dev/changesets.md). Feature [bsp-build-server.md](../../../docs/ft/coder/bsp-build-server.md). (tddy-build-typescript)
