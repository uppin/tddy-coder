...
**Location:** `packages/tddy-presenter/src/presenter/workflow_runner.rs:356` — `run_start_goal_without_output_dir`
**Moved:** 2026-09-23 — from `packages/tddy-core/src/presenter/workflow_runner.rs:356` by `#carve` 12/12 (PR #522), which carved `tddy-core` into a wiring point; the body moved unchanged, so the metrics below still hold
**Category:** complexity
...
| 2026-09-23 | 275 | 6 | 30 | 8 | **unchanged** — measured in the new home at wrap: the body is line-for-line identical to its `tddy-core` origin on `master` (`#carve` 12/14, #522 moved it with `git mv`) |
