...
**Location:** `packages/tddy-agent-backend/src/stream/cursor.rs:122` — `process_cursor_stream`
**Moved:** 2026-09-23 — from `packages/tddy-core/src/stream/cursor.rs:122` by `#carve` 12/12 (PR #522), which carved `tddy-core` into a wiring point; the body moved unchanged, so the metrics below still hold
**Category:** complexity
...
| 2026-09-23 | 167 | 11 | 32 | 1 | **unchanged** — measured in the new home at wrap: the body is line-for-line identical to its `tddy-core` origin on `master` (`#carve` 12/14, #522 moved it with `git mv`) |
