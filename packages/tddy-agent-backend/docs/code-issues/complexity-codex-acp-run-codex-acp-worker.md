...
**Location:** `packages/tddy-agent-backend/src/backend/codex_acp.rs:177` — `run_codex_acp_worker`
**Moved:** 2026-09-23 — from `packages/tddy-core/src/backend/codex_acp.rs:177` by `#carve` 12/12 (PR #522), which carved `tddy-core` into a wiring point; the body moved unchanged, so the metrics below still hold
**Category:** complexity
...
| 2026-09-23 | 244 | 13 | 24 | 1 | **unchanged** — measured in the new home at wrap: the body is line-for-line identical to its `tddy-core` origin on `master` (`#carve` 12/14, #522 moved it with `git mv`) |
