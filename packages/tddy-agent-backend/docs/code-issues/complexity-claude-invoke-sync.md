# complexity: invoke_sync

**Location:** `packages/tddy-agent-backend/src/backend/claude.rs:436` — `invoke_sync`
**Moved:** 2026-09-23 — from `packages/tddy-core/src/backend/claude.rs:435` by `#carve` 12/12 (PR #522), which carved `tddy-core` into a wiring point; the body moved unchanged, so the metrics below still hold
**Category:** complexity
**Detected:** 2026-09-18 — targeted by `/jev-restructuring` sweep, measured by structural scan
**Metrics:** **330 lines** · **nesting depth 4** · 1 parameters · 39 branch/match lines · 7 early exits
**Thresholds breached:** length 330 > 60 (`/analyze-clean-code`)
...
The body is **330 lines**, 5.5x the 60-line ceiling at which `/analyze-clean-code` says a function must be refactored.

The function carries **39 branch or match lines** and **7 early exits**
(`return` / `?`). Its file is 840 lines total, 767 of them production, across 10 functions.

**How this was found.** `/jev-restructuring` ranked it 88 of 3,503 production units by
...
Bring it under the `/analyze-clean-code` thresholds — length 330 > 60 — by `extract_method`
