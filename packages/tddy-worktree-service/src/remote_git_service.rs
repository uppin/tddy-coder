...
pub fn git_argv_as_user(
    ...
    if tddy_daemon_kernel::privilege_drop::pty_requires_privilege_drop(
        target.uid,
        target.gid,
        current_uid,
        current_gid,
    ) {
        tddy_daemon_kernel::privilege_drop::wrap_argv_for_privilege_drop(
            &argv, target.uid, target.gid,
        )
    } else {
        argv
    }
