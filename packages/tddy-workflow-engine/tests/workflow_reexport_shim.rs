//! Acceptance tests: the `tddy-workflow-engine::workflow` re-export shim preserves all external
//! import paths after the `tddy-graph` extraction.
//!
...
//! reachable under `tddy_workflow_engine::workflow::*` after the types physically move to
//! `tddy_graph`. They also verify that `tddy_graph` and `tddy_workflow_engine::workflow` share the
//! same type identity — a value of type `tddy_graph::graph::Graph` must be accepted where
...
/// `tddy_workflow_engine::workflow::task` still exposes `BackendInvokeTask` and `EndTask` (and the
/// pure types) after the extraction, even though `BackendInvokeTask` is defined in
/// tddy-workflow-engine and the pure types move to tddy-graph.
#[test]
...
/// `tddy_workflow_engine::workflow::graph` still exposes `Graph`, `GraphBuilder`, and
/// `ElicitationEvent`.
#[test]
...
/// The root-level `tddy_workflow_engine::*` re-exports that proxy through the shim modules still
/// resolve. (The crate root's `pub use workflow::{…}` re-exports `WorkflowEngine`,
/// `find_git_root`, `ElicitationEvent`, `ExecutionResult`, `ExecutionStatus`, `WorkflowState`,
/// `workflow_engine_storage_dir`, `WORKFLOW_ENGINE_STORAGE_SUBDIR`, `GoalOptions`.)
#[test]
fn lib_root_reexports_resolve_through_shim() {
    // Given — these types/values are accessible at the tddy_workflow_engine root
...
/// `tddy_graph::graph::Graph` and `tddy_workflow_engine::workflow::graph::Graph` must be the
/// **same type** (type identity preserved by the re-export shim — not a type alias that breaks
/// trait impls). Both names resolve to the same Rust type via `pub use tddy_graph::graph::*;` in
/// the shim.
#[test]
fn graph_type_identity_is_shared_across_tddy_graph_and_tddy_workflow_engine() {
    ...
    // Then — the same value is accepted where `tddy_workflow_engine::workflow::graph::Graph` is
    // expected, with no conversion — proving both names refer to the same Rust type.
    let _: Graph = graph;
}
