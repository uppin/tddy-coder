...
/// The per-goal hints a recipe hands a backend. Defined in [`tddy_workflow::hints`]; republished
/// here for the callers that named this path.
pub use tddy_workflow::{GoalHints, PermissionHint};
...
{instructions}",
    ...
    /// ([`crate::workflow::start_goal_for_session_continue`](crate::workflow::start_goal_for_session_continue)). Default: skip when the computed
