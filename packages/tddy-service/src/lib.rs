...
pub mod proto {
    ...
    /// The `worktree.activity` broadcast payload. Messages only — the events are published on a
    /// data-channel topic, so there is no service type here to serve or call.
    #[allow(unused_imports, unused_variables)]
    pub mod worktree_activity {
        include!(concat!(env!("OUT_DIR"), "/worktree_activity.rs"));
    }
