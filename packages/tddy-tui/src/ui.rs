...
/// Returns a short display prefix for a workflow session id (first two UUID hyphen groups when the
/// id matches that shape, otherwise [`SESSION_SEGMENT_PLACEHOLDER`]) or the documented placeholder
...
///
/// For a standard UUID string (`8hex-4hex-…`), returns those first two groups lowercased (e.g.
/// `019d390c-ac3e`). A lone `8hex` segment with no second group still returns that field. Missing,
/// empty, or malformed ids use [`SESSION_SEGMENT_PLACEHOLDER`] consistently.
pub fn first_hyphen_segment_of_workflow_session_id(session_id: Option<&str>) -> Cow<'static, str> {
    ...
    if parts.len() >= 2 {
        ...
        {
            let s = format!("{}-{}", a.to_ascii_lowercase(), b.to_ascii_lowercase());
            log::trace!("workflow session segment: extracted UUID prefix {:?}", s);
            return Cow::Owned(s);
        }
    }

    let first_field = parts.first().copied().unwrap_or(raw).trim();
...
mod tests {
    ...
    fn status_bar_text_orders_spinner_segment_then_goal() {
        ...
        assert!(
            spin < seg && seg < goal,
            "expected order / … 550e8400-e29b … Goal:, got {line:?}"
        );
    }
}
