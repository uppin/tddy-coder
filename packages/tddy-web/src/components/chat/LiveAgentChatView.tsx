...
import { bubbleClass } from "./chatEntryPresentation";
...
export interface LiveAgentChatViewProps {
...
/**
 * The interactive half of {@link AgentChatView}: a live conversation with the session's agent. The
 * transcript is one bubble per message, and below it either the free-text composer or — while the
 * agent is waiting on the operator — the clarification prompt it raised (select, multi-select, and
 * an "Other" free-text answer where the question allows one).
 */
export function LiveAgentChatView({
  placeholder,
  roomStatus = "idle",
  roomError = null,
  chat,
}: LiveAgentChatViewProps) {
  const {
    messages,
    elicitations,
    sendPrompt,
