...
**Thresholds breached:** length 121 > 60; nesting 8 > 4; parameters 6 > 5 (`/analyze-clean-code`)
...
The body is **121 lines**, 2.0x the 60-line ceiling at which `/analyze-clean-code` says a function must be refactored.

The function carries **11 branch or match lines** and **3 early exits**
(`return` / `?`). Its file is 240 lines total, 240 of them production, with **no `#[cfg(test)]` block**, across 0 functions.

**How this was found.** `/jev-restructuring` ranked it 20 of 3,503 production units by
...
Bring it under the `/analyze-clean-code` thresholds — length 121 > 60; nesting 8 > 4; parameters 6 > 5 — by `extract_method`
