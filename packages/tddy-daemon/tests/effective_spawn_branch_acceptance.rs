...
fn an_unset_intent_is_keyed_on_the_new_branch_name() {
    ...
    // When
    let branch = effective_spawn_branch("", "feature/attach-docs/attach-start", "");
...
fn an_existing_branch_spawn_is_keyed_on_the_local_branch_behind_a_remote_tracking_name() {
    ...
    // names (`list_recent_remote_branches` reads `refs/remotes/origin`), so this is the shape the
