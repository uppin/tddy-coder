...
> **[`tddy-session-activity`](../../tddy-session-activity/docs/activity-service.md)**, the tools-and-agents
> catalogue in **[`tddy-discovery`](../../tddy-discovery/docs/catalog-service.md)**, exec-tool execution
> in **[`tddy-tool-engine`](../../tddy-tool-engine/README.md)**, and PR-stack planning RPCs in
> **[`pr-stack-service.md`](pr-stack-service.md)** — each serving a coordinate of its own beside this one.
> `connection_service` stays here and wires them all.
...
| `catalog.CatalogService` | `ListTools`, `ListAgents`, `ListAgentModels`, `ListSubagents` | [`packages/tddy-discovery`](../../tddy-discovery/docs/catalog-service.md) |
| `exec_tools.ExecToolService` | `ExecuteTool`, `StreamExecuteTool`, `ListExecTools`, `ListSessionToolCalls` | [`packages/tddy-tool-engine`](../../tddy-tool-engine/README.md) |
| `pr_stack.PrStackService` | `AddPlannedPr`, `GetPrStatus`, `RepointPlannedPr`, `ReorderPlannedPr`, `PullBaseIntoBranch`, `QueryBranch`, `ResolveStackBase`, `LinkStackNode` | [pr-stack-service.md](pr-stack-service.md) |

`connection.ConnectionService` serves **17** methods — session lifecycle, projects and branches, demo
VM control, worktree snapshots for session rooms, local token minting, and streaming session start.

A client reaches all eleven sibling services at the same coordinates — `/rpc` over Connect-HTTP, the
LiveKit common room, and the local UDS socket — because each is a `ServiceEntry` registered beside
this one rather than a second endpoint. The daemon's wiring builds every one of them from the same
managers over the same `Arc`s, so two coordinates can never address two sets of state.
...
| `ListProjectBranches` | Lists branch names for a project checkout (local registry row → `main_repo_path`). |
...
| `StartDemoVm` / `StopDemoVm` / `GetDemoVmStatus` | UI-driven QEMU demo VM lifecycle (family O). |
| `GetWorktreeSnapshot` | One measurement of a session checkout for the daemon hosting that session's room; forwards to the codebase daemon when the worktree is remote. |
| `MintLocalToken` | Mint a session token for the Unix-socket peer's uid (family Q; UDS transport only). |
| `StreamStartSession` | Streaming twin of `StartSession` with per-attachment materialization progress; see [Start-session attachments](#start-session-attachments). |

Exec-tool, catalogue and PR-stack RPCs are documented on the sibling services above.
