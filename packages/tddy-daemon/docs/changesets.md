...
- **2026-03-28** [Feature] Multi-host daemon selection (phase 1) — `ConnectionService`: `ListEligibleDaemons` via `EligibleDaemonSource`; `ListSessions` populates `daemon_instance_id`; `StartSession` honors local vs non-local `daemon_instance_id` (non-local returns unimplemented until peer routing). Feature docs: [connection-service.md](./connection-service.md), [docs/ft/web/web-terminal.md](../../../docs/ft/web/web-terminal.md). (tddy-daemon, tddy-service, tddy-web)
