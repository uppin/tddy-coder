...
/// belonging to any one connection — see [`tddy_session_lifecycle::oauth_loopback_tunnel::spawn_oauth_loopback_tunnel`].
...
/// [`tddy_session_lifecycle::common_room_supervisor`], which stops and restarts it when the configured room changes,
...
/// Every family reachable on the binary's local Unix-domain socket, in one bundle.
type BinaryLocalSocketServices = crate::local_socket_server::LocalSocketServices<
...
struct LocalSocketTransport {
    socket_path: PathBuf,
    /// Node 1 added hosts and worktrees, node 6 the terminal family and node 7 the session-agent
    /// and activity families; node 8 catalog, exec tools and PR stack. A caller that reached a
    /// method here before its split must go on reaching it here after. Adapters are generated by
    /// `tddy-codegen`, not hand-written.
    services: BinaryLocalSocketServices,
}
...
pub async fn build(
    ...
    // The common room this daemon joins, if any, and its roster. Built before auth because the
    // roster is also where peers' session-token keys are read from: a token a peer minted is
    // verified against the key that peer advertises here. Resolved once, so the key directory and
    // peer discovery below cannot disagree about whether there is a room.
    let common_room_peers = CommonRoomTarget::from_livekit(config.livekit.as_ref()).map(|target| {
    ...
    let auth_result = match &signing_key {
        Some(key) => {
            let directory: Arc<dyn tddy_daemon_auth::KeyDirectory> = match &common_room_peers {
                Some((_, registry)) => Arc::new(
                    crate::common_room_key_directory::CommonRoomKeyDirectory::new(Arc::clone(
                        registry,
                    )),
                ),
            ...
            log::info!(
                target: tddy_daemon_auth::AUTH_LOG_TARGET,
    ...
    // What the common room is told about this daemon's key, so a peer holding one of its tokens
    // can find the key that verifies it.
    let advertised_signing_key = signing_key
        .as_ref()
        .map(crate::common_room_key_directory::advertised_signing_key);
    ...
    let config_service_authenticator = tddy_session_lifecycle::auth::session_token_authenticator(
        auth_result.user_resolver.as_ref(),
    ...
    if let (Some(user_resolver), Some(session_tokens), Some(advertised_signing_key)) = (
        auth_result.user_resolver,
        auth_result.session_tokens,
        advertised_signing_key,
    ) {
        ...
        if let Err(e) = host_registry.record_sighting(
            &tddy_session_lifecycle::host_registry::local_host_sighting(&config),
            tddy_session_lifecycle::host_registry::now_unix_ms(),
        ) {
            ...
            Some((target, registry)) => {
                ...
                Some(tddy_daemon_livekit::livekit_peer_discovery::LiveKitDiscoveryHandles {
                        eligible_daemon_source: Arc::new(
                            ...
                            as Arc<dyn tddy_session_lifecycle::multi_host::EligibleDaemonSource>,
        ...
        // Built here, before the resolvers are moved into DaemonSessionHost below.
        ...
        let session_notification_bus = {
            ...
            Arc::new(bus.with_subscriber(Arc::new(
                tddy_session_lifecycle::session_notification_subscribers::SessionNotificationStreamSubscriber::new(),
            )))
        };
        ...
        // this one.
        let host_prompts: Arc<dyn tddy_session_lifecycle::host_prompts::HostPromptRegistry> =
            Arc::new(tddy_session_lifecycle::host_prompts::InMemoryHostPromptRegistry::new());

        // Built before `DaemonSessionHost` consumes `livekit_discovery`: the host service
        ...
        if let Some(vaults) = auth_result.credential_vaults.clone() {
            tddy_daemon_auth::vault_lifetimes::spawn_credential_sweep(&vaults);
        ...
        if options.host == RuntimeHost::Binary {
            ...
            tasks.local_socket = Some(LocalSocketTransport {
                ...
                services: crate::local_socket_server::LocalSocketServices {
                    session: tddy_service::proto::session::SessionServiceTonicAdapter::new(
                    ...
                    // registry the `terminal_session_entry` below serves, so the socket and every
                    // other transport address one set of PTYs and one control lease. The ports
                    // themselves hold no state: they are clones of those managers' `Arc`s.
                    terminal:
                    ...
                    // Likewise built from `connection_arc`, so the roster an in-jail agent reaches
                    // over this socket is the one a browser sees over HTTP. These two are the
                    // reason the socket's service list is checked by an acceptance test: five of
                    // the nine family-B methods are what `tddy-sandbox-runner`'s relay allowlist
                    // permits, and a coordinate missing here fails closed inside a jail.
                    session_agents:
                        tddy_service::proto::session_agents_svc::SessionAgentServiceTonicAdapter::new(
                            Arc::new(connection_arc.session_agents_service()),
                        ),
        ...
        // then removed the same nine from `the pre-unbundle monolithic RPC coordinate`, so this is the only
        ...
        // and then removed the same thirteen from `the pre-unbundle monolithic RPC coordinate`, so this is the
        ...
        // mounted this coordinate and then removed the same nine from
        // `the pre-unbundle monolithic RPC coordinate`, so this is the only place they answer. The entry is
        // wrapped here rather than in the crate because seven of the nine route by
        ...
        let task_service_impl = tddy_session_lifecycle::task_service::TaskServiceImpl::new(
            task_registry.clone(),
            vm_user_resolver.clone(),
        );
        ...
        let action_service_impl = tddy_session_lifecycle::action_service::ActionServiceImpl::new(
            task_registry.clone(),
...
fn build_telegram(
    ...
    tddy_data_dir: &Path,
    options: &RuntimeOptions,
    claude_cli_manager: &Arc<tddy_session_lifecycle::cli_session_manager::CliSessionManager>,
