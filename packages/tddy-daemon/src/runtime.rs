...
struct LocalSocketTransport {
    socket_path: PathBuf,
    /// Every family reachable on this socket, in one bundle. Node 1 added hosts and worktrees,
    /// node 6 the terminal family and node 7 the session-agent and activity families; a caller
    /// that reached `GetHostTooling` — or an in-jail agent that reached `OpenAgentConversation` —
    /// here before its split must go on reaching it here after. All of the moved families'
    /// adapters are generated by `tddy-codegen`, not hand-written.
