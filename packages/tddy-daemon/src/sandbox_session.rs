...
/// Discovery-subagent configuration for a sandboxed claude-cli session, threaded from
/// `StartSessionRequest` (new session) or persisted `SessionMetadata` (resume). Mirrors
/// `tddy_sandbox_app::spawn::SubagentSpawnConfig` — kept as a separate daemon-side type because
/// that one is `pub(crate)` to its own crate and `tddy-sandbox-app` depends on `tddy-daemon`, not
/// the reverse.
...
pub struct SubagentSpawnConfig {
    ...
    /// `--subagent-replaces`-equivalent override (comma-separated tool names). `None` means "use
    /// the subagent's declared default" — resolved via
    /// [`tddy_discovery::subagent::resolve_replaced_tools`].
    pub replaces: Option<String>,
}

/// Builds the `TDDY_SUBAGENT_*` env overlay for the in-jail `tddy-tools --mcp` process. Empty when
/// no subagent is configured. Only sets a `FASTCONTEXT_*`/`REPLACES` key for a field that was
/// actually given — env-var defaulting for absent fields happens on the reading side
/// (`tddy-tools`/`tddy-sandbox-runner`), not here.
pub fn subagent_env_overlay(
...
mod subagent_env_overlay_tests {
    ...
    //
    // Feature: docs/ft/coder/managed-codebase-subagents.md § Tool replacement (criterion 18)
    // Changeset: docs/dev/1-WIP/2026-07-02-changeset-subagent-tool-replacement.md
    //
    // Daemon-side port of tddy_sandbox_app::spawn::subagent_env_overlay's contract.
    ...
    /// With a discovery subagent and every FastContext flag given, all of them are threaded into
    /// the `TDDY_SUBAGENT_*` env overlay.
    #[test]
    ...
    /// With `replaces: None`, the overlay never invents the per-subagent default —
    /// `TDDY_SUBAGENT_REPLACES` is simply absent, exactly like the `TDDY_SUBAGENT_FASTCONTEXT_*`
    /// fields when their source field is unset.
    #[test]
    ...
    /// With an explicit `replaces` override, the overlay carries the normalized, CSV-joined
    /// resolved set — not the raw override string.
    #[test]
