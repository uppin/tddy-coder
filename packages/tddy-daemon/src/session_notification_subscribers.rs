...
//!
//! The stream subscriber `tddy-web` reads moved to
//! [`tddy_session_activity::session_notification_subscribers`] with `#unbundle` node 7 and is
//! re-exported below. This one stayed: it delivers through [`TelegramDaemonHooks`] and reads the
//! daemon's `telegram:` config block, so it is a subscriber *of* the daemon rather than part of
