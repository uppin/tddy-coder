...
//! only the fields that family reads. These hand those fields out: the shared ones (`Arc`s) as
//! clones of the same handle, so a handler talks to the peers, spawn client and common room the
//! host does rather than to copies of them.
