...
impl DaemonSessionHost {
    ...
    pub(crate) async fn relaunch_sandboxed_runner(
        ...
        let (sandbox_root, egress_dir, scratch_dir, scratch_tmp, context_dir) =
            prepare_relaunch_dirs(session_dir)?;

        refresh_relaunch_context_dir(worktree_path, agents, &context_dir)?;
    ...
    async fn spawn_relaunched_runner(
        &self,
    ...
    async fn bridge_relaunched_jail(
        &self,
...
fn refresh_relaunch_context_dir(
    worktree_path: &Path,
    agents: &[tddy_core::SessionAgentRecord],
    context_dir: &PathBuf,
) -> Result<(), Status> {
