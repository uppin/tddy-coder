...
pub(crate) async fn spawn_claude_cli_session_inner(
    ...
    let worktree_path = cut_claude_cli_worktree(ClaudeCliWorktreeCut {
        config,
        session_id,
    ...
    let spawned_branch = spawned_claude_cli_branch(
        branch_worktree_intent,
        new_branch_name,
        selected_branch_to_work_on,
        project_id,
    ...
    let (tddy_tools_path, hook_token) =
        install_claude_cli_hooks(config, os_user, session_id, &worktree_path);
    ...
    let (managed, append_system_prompt_file, env_extra) =
        managed_claude_cli_launch(ManagedClaudeCliLaunch {
            tddy_data_dir,
            session_id,
    ...
    let handle = spawn_claude_cli_process(ClaudeCliProcess {
        os_user,
        session_id,
    ...
    let (lk_room, lk_url, lk_server_identity) = claude_cli_livekit_room(config, session_id);
...
async fn cut_claude_cli_worktree(launch: ClaudeCliWorktreeCut<'_>) -> Result<PathBuf, Status> {
...
struct ManagedClaudeCliLaunch<'a> {
    tddy_data_dir: &'a Path,
    session_id: &'a str,
    managed_recipe: &'a Option<Arc<dyn tddy_core::workflow::recipe::WorkflowRecipe + 'static>>,
    child_spawn_handler: Option<Arc<dyn tddy_core::toolcall::ChildSpawnHandler + 'static>>,
    conversation_spawn_handler:
...
async fn managed_claude_cli_launch(
...
fn claude_cli_livekit_room(config: &DaemonConfig, session_id: &str) -> (String, String, String) {
