...
async fn report_pre_then_post_tool_use_coalesces_into_one_completed_call() {
    ...
    service
        .activity_service()
        .report_agent_activity(Request::direct(a_pre_tool_use(
            session_id,
            "Bash",
    ...
    service
        .activity_service()
        .report_agent_activity(Request::direct(a_post_tool_use(
            session_id,
            "Bash",
...
async fn report_agent_activity_rejects_bad_hook_token() {
    ...
    let err = service
        .activity_service()
        .report_agent_activity(Request::direct(req))
        .await
        .unwrap_err();
...
async fn stream_session_activity_replays_the_persisted_snapshot() {
    ...
    let mut stream = service
        .activity_service()
        .stream_session_activity(Request::direct(StreamSessionActivityRequest {
...
async fn stream_acp_replay_replays_the_persisted_snapshot() {
    ...
    let mut stream = service
        .activity_service()
        .stream_acp_replay(Request::direct(StreamAcpReplayRequest {
            session_token: "valid".to_string(),
            session_id: session_id.to_string(),
            daemon_instance_id: String::new(),
            mode: StreamMode::SnapshotThenLive as i32,
...
async fn get_acp_tool_call_detail_returns_the_full_tool_bodies() {
    ...
    let detail = service
        .activity_service()
        .get_acp_tool_call_detail(Request::direct(GetAcpToolCallDetailRequest {
            session_token: "valid".to_string(),
            session_id: session_id.to_string(),
            daemon_instance_id: String::new(),
...
async fn get_acp_replay_page_serves_the_frames_before_the_cursor() {
    ...
    let page = service
        .activity_service()
        .get_acp_replay_page(Request::direct(GetAcpReplayPageRequest {
            session_token: "valid".to_string(),
            session_id: session_id.to_string(),
            daemon_instance_id: String::new(),
            before_seq: 3,
