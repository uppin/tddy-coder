//! gRPC client: subscribe to a child `tddy-coder`'s [`PresenterObserver`] and drive the sinks that
//! care about its events — an injected [`PresenterEventSink`] (Telegram's chat surface, on a daemon
//! that has one) and the session-notification bus.
//!
...
///
/// Both sinks are optional and independent. `sink` drives the chat surface — elicitation
...
pub fn spawn_presenter_observer_task(
    sink: Option<SharedPresenterEventSink>,
    notifications: Option<SessionNotificationPublishing>,
    session_id: &str,
    grpc_port: u16,
) {
    if sink.is_none() && notifications.is_none() {
        log::debug!(
    ...
    tokio::spawn(async move {
        match run_presenter_observer_loop(sink, notifications, session_id, grpc_port).await {
            Ok(()) => {}
            Err(e) => {
                log::warn!(
...
async fn run_presenter_observer_loop(
    ...
    while let Some(result) = stream.message().await? {
        if let Some(ref sink) = sink {
            sink.on_presenter_event(&session_id, &result).await?;
        }
