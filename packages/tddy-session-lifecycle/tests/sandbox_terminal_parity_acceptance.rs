...
/// reach it through.
///
/// Built from [`tddy_session_lifecycle::test_util::test_service`] and the daemon's own
...
struct ADaemonServingAJailedSession {
    ...
    _daemon: tddy_session_lifecycle::test_util::TestDaemon,
...
impl ADaemonServingAJailedSession {
    async fn running_a_jail() -> Self {
        ...
        let daemon = tddy_session_lifecycle::test_util::test_service(data_dir.path().to_path_buf());
        daemon
            .sandbox_sessions()
            .insert(
