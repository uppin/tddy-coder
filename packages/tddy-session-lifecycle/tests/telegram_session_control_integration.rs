...
//! Integration tests for [`tddy_session_lifecycle::telegram_session_control`] (harness + sender recording).
...
use tddy_daemon_kernel::config::{AllowedAgent, DaemonConfig};
...
fn harness_with_workflow_projects(
    ...
    let workflow_spawn = Arc::new(TelegramWorkflowSpawn {
        ...
        claude_cli_manager: Arc::new(
            tddy_session_lifecycle::claude_cli_session::ClaudeCliSessionManager::new(),
        ),
    });
...
async fn telegram_more_recipes_sends_second_keyboard() {
    ...
    assert_eq!(
        mr_buttons.len(),
        tddy_session_lifecycle::telegram_session_control::RECIPE_MORE_PAGE.len(),
