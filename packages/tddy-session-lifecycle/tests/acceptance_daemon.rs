...
use tddy_session_lifecycle::session_reader;
...
fn acceptance_project_storage_roundtrip() {
    ...
    let p = tddy_projects::project_storage::ProjectData {
    ...
    // When
    tddy_projects::project_storage::add_project(&projects_dir, p.clone()).unwrap();
    let list = tddy_projects::project_storage::read_projects(&projects_dir).unwrap();
