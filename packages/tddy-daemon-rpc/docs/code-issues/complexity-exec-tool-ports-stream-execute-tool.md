# complexity: stream_execute_tool

**Location:** `packages/tddy-daemon-rpc/src/exec_tool/ports.rs:99` — `stream_execute_tool` (now on `ExecToolRpcHandler`)
**Moved:** 2026-09-23 by #520 (#carve 11) — from `packages/tddy-session-lifecycle/src/connection_service/svc_exec_tool_ports.rs:84`; body unchanged except `self.x` → the handler's fields and components, which rustfmt re-wraps (73 → 82 lines)
**Category:** complexity
**Detected:** 2026-09-18 — targeted by `/jev-restructuring` sweep, measured by structural scan
**Metrics:** **73 lines** · **nesting depth 5** · 1 parameters · 6 branch/match lines · 8 early exits
...
|---|---|---|---|---|---|
| 2026-09-18 | 73 | 5 | 6 | 8 | first detection |
| 2026-09-23 | 82 | 5 | 6 | 8 | moved to `tddy-daemon-rpc`; length from re-wrapped field paths, not new logic |
| 2026-09-23 | 82 | 5 | 6 | 8 | re-measured at the #520 wrap: unchanged since the move; still at `exec_tool/ports.rs:99`, nesting and early exits identical to the lifecycle original |
...
The function carries **6 branch or match lines** and **8 early exits**
(`return` / `?`). Its file is 326 lines total, 326 of them production, with **no `#[cfg(test)]` block**, across 4 functions.

**How this was found.** `/jev-restructuring` ranked it 49 of 3,503 production units by
